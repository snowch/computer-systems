...
census is a prediction committed to before the measurement exists, and ch20 is about what
...
def refuse_a_census_that_no_longer_describes_the_chapter(facts: dict[str, Any]) -> None:
    if not facts["agree"]:
        raise CensusError(
            ...
            "the same program written twice; ch20 compares their profiles, and a comparison "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not differences:
        print(f"{payload['name']}: unchanged — the program still has the shape ch20 describes")
        return 0
