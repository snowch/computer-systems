...
READS: dict[str, tuple[str, str]] = {
    "entry.S": ("ch08", "the first instructions the kernel runs, before there is a stack"),
    "start.c": ("ch08", "the machine-mode setup that hands over to supervisor mode"),
    "trampoline.S": ("ch08", "uservec and userret — the register moves ch08 counts"),
    "trap.c": ("ch08", "where a trap is decided, and where a page fault would be handled"),
    "syscall.c": ("ch08", "the dispatch table, and how arguments cross the boundary"),
    "syscall.h": ("ch08", "the call numbers"),
    "sysproc.c": ("ch08", "the process calls, including the one ch08's workload uses"),
    "vm.c": ("ch09", "the page-table walk, mapping, and the kernel's own address space"),
    "riscv.h": ("ch09", "the Sv39 field definitions, as macros over a 64-bit word"),
    "memlayout.h": ("ch09", "what is mapped where, and why the trampoline is at the top"),
    "kalloc.c": ("ch10", "the physical page allocator a fault ends up calling"),
    "exec.c": ("ch10", "what a fresh address space is built out of"),
    "kernelvec.S": ("ch11", "the trap path taken when the kernel itself is interrupted"),
    "plic.c": ("ch11", "which device is allowed to interrupt which hart"),
    "console.c": ("ch11", "a device driver small enough to read in full"),
    "uart.c": ("ch11", "the registers underneath it"),
    "spinlock.c": ("ch12", "the lock, and the memory barriers around it"),
    "spinlock.h": ("ch12", "what a lock is made of"),
    "sleeplock.c": ("ch12", "the other kind, and when each is correct"),
    "proc.c": ("ch13", "the scheduler, and both halves of a context switch"),
    "proc.h": ("ch13", "the context ch13 counts the registers of"),
    "swtch.S": ("ch13", "the switch itself — fourteen registers and nothing else"),
    "bio.c": ("ch14", "the buffer cache, and why a read can cost nothing"),
    "log.c": ("ch14", "the write-ahead log ch14 counts the amplification of"),
    "fs.c": ("ch14", "inodes, blocks, and the path from a name to a byte"),
    "file.c": ("ch14", "what a file descriptor actually is"),
    "virtio_disk.c": ("ch14", "the only device in the system that makes you wait"),
}
