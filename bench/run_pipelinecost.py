...
**The branch sweep uses `sysfs_count_over_calling` and not `sysfs_count_over`.** ch19 opens by
...
FIGURE = "ch19-ilp and ch19-branches"
...
class PipelineCostError(RuntimeError):
    """The sweep stopped measuring what ch19 is about."""
...
def capture() -> dict[str, Any]:
    ...
    for width in CHAINS:
        ...
        if "cycles" not in counts or "instructions" not in counts or not counts["cycles"]:
            raise PipelineCostError(
                ...
                "ch19's first table is IPC; it is not a figure this book will estimate."
            )
    ...
    if len(answers) != 1:
        raise PipelineCostError(
            ...
            "with a different number of accumulators, and ch19 compares them only while that holds."
        )
    ...
    for predictable in PREDICTABILITY:
        ...
        if "branch-misses" not in counts or "branches" not in counts:
            raise PipelineCostError(
                ...
                "measured. ch19 says what it would take rather than estimating it."
            )
        points.append(
            {
    ...
    if spread < 1:
        raise PipelineCostError(
            ...
            "compiler removed the branch — ch19 shows it doing exactly that to the other "
