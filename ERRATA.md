...
|---|---|---|---|
| 2026-09-14 | [ch00](#prerequisites-and-setup), PLAN §5 | Part V was planned on RISC-V hardware. No purchasable RISC-V core both counts and samples, so [ch30](#whole-machine-profiling) and [ch31](#vectors) would have been unmeasurable, on boards that are hard to buy. | Part V moved to AArch64; the evidence and the cost are recorded in `hardware/README.md` and PLAN §5, and every chapter with an `assumes` field in `bench/outline.py` says what it assumes in its own header. |
