...
|---|---|---|
| The board has not reported | `setting-up-the-board-report` figure, `bench/figures.py` | `bench/results/setup-host.json` does not exist yet. Run `make bench-board` on the reference machine, commit the result, and remove the `pending=` marker. |
