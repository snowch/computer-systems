...
the point [ch04](#ch04) makes is that a convention is a contract and an interrupt is not a party
to it.
...
to do it, which is most of why [ch04](#ch04)'s `-O2` listings have no stack frame at all.
...
promise binds the hardware, which is why [ch06](#ch06)'s trap path saves all thirty-one.
...
|---|---|---|
| `stvec` | The address the hardware jumps to on a supervisor trap | [ch06](#ch06) |
| `sepc` | The PC at the moment of the trap; `sret` returns here | [ch06](#ch06) |
| `scause` | Why the trap happened, as a code with the interrupt bit at the top | [ch06](#ch06), [ch09](#ch09) |
| `sstatus` | Supervisor status: previous privilege, previous interrupt-enable | [ch06](#ch06), [ch09](#ch09) |
| `sscratch` | One word the trap path may use before it has a usable register | [ch06](#ch06) |
| `stval` | The faulting address, for a fault that has one | [ch08](#ch08) |
| `satp` | The root page table and the translation mode | [ch07](#ch07) |
| `sie` / `sip` | Which interrupts are enabled, and which are pending | [ch09](#ch09) |
| `time` | The real-time counter, readable from user mode | [ch09](#ch09) |
...
The rest of the field means different things in the two cases, and [ch09](#ch09) is about why the
...
to put something that survives the swap, and [ch06](#ch06) traces exactly what xv6 keeps there.

## Sv39, in one place

The paging scheme this book uses. [ch07](#ch07) derives it and measures the walk; this is the
...
@riscv-sbi describes the layer beneath the kernel that [ch06](#ch06) mentions and this book does
not otherwise use.
