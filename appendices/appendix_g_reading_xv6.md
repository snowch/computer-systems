...
|---|---|---|
| Operating system interfaces — processes, `fork`, `exec`, files | [ch02](#c-without-a-runtime) for the memory model it implies; the trace below | — |
| Operating system organization, isolation, privilege | [ch13](#traps-and-system-calls) | [ch26](#the-os-layers-cost) |
| Page tables and Sv39 | [ch14](#virtual-memory) | [ch22](#the-memory-hierarchy) address translation, [ch26](#the-os-layers-cost) faults |
| Traps, system calls, and the trap path | [ch13](#traps-and-system-calls) | [ch26](#the-os-layers-cost) |
| Page faults and what can be built on them | [ch15](#page-faults-as-a-feature) | [ch26](#the-os-layers-cost) |
| Interrupts and device drivers | [ch16](#interrupts-and-drivers) | — |
| Locking | [ch17](#locks-and-memory-ordering) | [ch25](#memory-ordering-on-real-hardware) |
| Scheduling and context switching | [ch18](#scheduling-and-context-switches) | [ch26](#the-os-layers-cost) |
| File system, logging, buffer cache | [ch19](#the-file-system) | — |
| Concurrency revisited, memory ordering | [ch17](#locks-and-memory-ordering) | [ch25](#memory-ordering-on-real-hardware) |
...
from. [ch16](#interrupts-and-drivers) and [ch19](#the-file-system) say so in their own limitations sections.
...
|---|---|---|
| `allocproc()` finds an unused slot in the fixed process table | allocation without a heap | [ch02](#c-without-a-runtime) |
| …and returns `0` when there is none, which becomes `-1` | failure that returns rather than raises | [ch02](#c-without-a-runtime), problem 2.2 |
| `uvmcopy()` walks the parent's page table and copies **every** page | address translation | [ch14](#virtual-memory) |
| …with a `kalloc()` per page, which can also fail | the physical allocator | [ch15](#page-faults-as-a-feature) |
| `*(np->trapframe) = *(p->trapframe)` copies the saved user registers | what a trap saves | [ch13](#traps-and-system-calls) |
| `np->trapframe->a0 = 0` | the calling convention | [ch11](#machine-level-code-on-riscv), [ch13](#traps-and-system-calls) |
| `filedup()` over the open files, and `idup()` on the working directory | reference counting | [ch19](#the-file-system) |
| `np->state = RUNNABLE` makes it eligible to be chosen | scheduling | [ch18](#scheduling-and-context-switches) |
...
its own `a0`. [ch13](#traps-and-system-calls) is where that saved register set is counted.
...
makes [ch15](#page-faults-as-a-feature)'s copy-on-write material a change rather than an explanation — the mechanism
is absent here, so the chapter has somewhere to put it. A production kernel shares the pages and
marks them read-only, and [ch26](#the-os-layers-cost) is where the difference in cost is measured.
...
caller that does not look at the result has written the bug [ch02](#c-without-a-runtime) is about.
