...
| Operating system organization, isolation, privilege | [ch14](#traps-and-system-calls) | [ch27](#the-os-layers-cost) |
| Page tables and Sv39 | [ch15](#virtual-memory) | [ch23](#the-memory-hierarchy) address translation, [ch27](#the-os-layers-cost) faults |
| Traps, system calls, and the trap path | [ch14](#traps-and-system-calls) | [ch27](#the-os-layers-cost) |
| Page faults and what can be built on them | [ch16](#page-faults-as-a-feature) | [ch27](#the-os-layers-cost) |
| Interrupts and device drivers | [ch17](#interrupts-and-drivers) | — |
| Locking | [ch18](#locks-and-memory-ordering) | [ch26](#memory-ordering-on-real-hardware) |
| Scheduling and context switching | [ch19](#scheduling-and-context-switches) | [ch27](#the-os-layers-cost) |
| File system, logging, buffer cache | [ch20](#the-file-system) | — |
| Concurrency revisited, memory ordering | [ch18](#locks-and-memory-ordering) | [ch26](#memory-ordering-on-real-hardware) |
...
from. [ch17](#interrupts-and-drivers) and [ch20](#the-file-system) say so in their own limitations sections.
...
| `uvmcopy()` walks the parent's page table and copies **every** page | address translation | [ch15](#virtual-memory) |
| …with a `kalloc()` per page, which can also fail | the physical allocator | [ch16](#page-faults-as-a-feature) |
| `*(np->trapframe) = *(p->trapframe)` copies the saved user registers | what a trap saves | [ch14](#traps-and-system-calls) |
| `np->trapframe->a0 = 0` | the calling convention | [ch12](#machine-level-code-on-riscv), [ch14](#traps-and-system-calls) |
| `filedup()` over the open files, and `idup()` on the working directory | reference counting | [ch20](#the-file-system) |
| `np->state = RUNNABLE` makes it eligible to be chosen | scheduling | [ch19](#scheduling-and-context-switches) |
...
its own `a0`. [ch14](#traps-and-system-calls) is where that saved register set is counted.
...
makes [ch16](#page-faults-as-a-feature)'s copy-on-write material a change rather than an explanation — the mechanism
is absent here, so the chapter has somewhere to put it. A production kernel shares the pages and
marks them read-only, and [ch27](#the-os-layers-cost) is where the difference in cost is measured.
