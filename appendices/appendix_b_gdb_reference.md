...
The sequence [ch14](#traps-and-system-calls) is written around:
...
address; [ch16](#page-faults-as-a-feature) is where it stops being zero.
...
number of the root page table, which is where [ch15](#virtual-memory)'s walk starts.
...
run, `sp` still belongs to the interrupted program, and the calling convention [ch12](#machine-level-code-on-riscv)
...
order, and [ch25](#the-cpu) is the chapter about the difference.
