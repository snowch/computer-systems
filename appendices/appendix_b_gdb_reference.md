...
The sequence [ch13](#ch13) is written around:
...
address; [ch15](#ch15) is where it stops being zero.
...
number of the root page table, which is where [ch14](#ch14)'s walk starts.
...
run, `sp` still belongs to the interrupted program, and the calling convention [ch11](#ch11)
...
the debugger, and this is the `xv6` target, where a duration means nothing anyway ([ch00](#ch00)).
...
order, and [ch24](#ch24) is the chapter about the difference.
