...
[ch04](#ch04) and is about to read [ch16](#ch16), and it is organised as *you know this already,
...
Part I and Part II are RISC-V because the kernel small enough to read in an afternoon is a RISC-V
kernel. Part III is AArch64 because that is where the performance counters work: sampling needs a
PMU that can raise an interrupt on counter overflow, and no affordable RISC-V core does both.
[ch00](#ch00) has the evidence and [ch18](#ch18) has the return — a reader shown one weak memory
...
in [ch16](#ch16)'s listings wherever an `int` is involved, and the zero-extension is free rather
...
the stack pointer, in one instruction. The `!` is the write-back. [ch04](#ch04)'s RISC-V prologues
...
with no branch at all. [ch17](#ch17) is about what that is worth: a branch the predictor cannot
...
[ch10](#ch10) prints both of these from real disassembly and [ch18](#ch18) is the chapter about
...
which is exactly why [ch18](#ch18) puts them side by side rather than teaching one.

Both spell the same requirement. Neither spelling is the concept.

## Vectors

NEON, in one paragraph, because [ch21](#ch21) is the chapter.
...
That last point is a trap [ch21](#ch21) fell into and records: an instruction naming a `v`
...
the MMU, system registers — all different, none of it here, because Part III does not read the
kernel and Part II does not run on AArch64.
