...
[ch12](#machine-level-code-on-riscv) and is about to read [ch24](#optimising-code), and it is organised as *you know this already,
...
in [ch24](#optimising-code)'s listings wherever an `int` is involved, and the zero-extension is free rather
...
every listing in [ch24](#optimising-code).
...
the stack pointer, in one instruction. The `!` is the write-back. [ch12](#machine-level-code-on-riscv)'s RISC-V prologues
...
vectorised loops in [ch29](#vectors).
...
with no branch at all. [ch25](#the-cpu) is about what that is worth: a branch the predictor cannot
...
[ch18](#locks-and-memory-ordering) prints both of these from real disassembly and [ch26](#memory-ordering-on-real-hardware) is the chapter about
...
which is exactly why [ch26](#memory-ordering-on-real-hardware) puts them side by side rather than teaching one.

Both spell the same requirement. Neither spelling is the concept.

## Vectors

NEON, in one paragraph, because [ch29](#vectors) is the chapter.
...
That last point is a trap [ch29](#vectors) fell into and records: an instruction naming a `v`
