...
[ch11](#machine-level-code-on-riscv) and is about to read [ch23](#optimising-code), and it is organised as *you know this already,
...
[ch00](#prerequisites-and-setup) has the evidence and [ch25](#memory-ordering-on-real-hardware) has the return — a reader shown one weak memory
...
in [ch23](#optimising-code)'s listings wherever an `int` is involved, and the zero-extension is free rather
...
every listing in [ch23](#optimising-code).
...
the stack pointer, in one instruction. The `!` is the write-back. [ch11](#machine-level-code-on-riscv)'s RISC-V prologues
...
vectorised loops in [ch28](#vectors).
...
with no branch at all. [ch24](#the-cpu) is about what that is worth: a branch the predictor cannot
...
[ch17](#locks-and-memory-ordering) prints both of these from real disassembly and [ch25](#memory-ordering-on-real-hardware) is the chapter about
...
which is exactly why [ch25](#memory-ordering-on-real-hardware) puts them side by side rather than teaching one.

Both spell the same requirement. Neither spelling is the concept.

## Vectors

NEON, in one paragraph, because [ch28](#vectors) is the chapter.
...
That last point is a trap [ch28](#vectors) fell into and records: an instruction naming a `v`
