...
[ch15](#ch15) established that the structural model does not predict cost, and pointed at a
...
```{include} _generated/ch16-clock.md
```
...
```{include} _generated/ch16-spread.md
```
...
The first few measurements are slower, always, and for reasons Parts II and III have already
explained: [ch10](#ch10)'s pages are not yet faulted in, [ch17](#ch17)'s caches hold somebody
else's data, and the branch predictor of [ch19](#ch19) has never seen this loop.
...
```{include} _generated/ch16-bias.md
```
...
[ch11](#ch11)'s subject arriving where it is least wanted.
...
answer is to find out what distinguishes them, which is [ch22](#ch22)'s equipment.
...
later uses — is stated rather than demonstrated. [ch18](#ch18) shows what happens when it is
forgotten, which is a benchmark that measures an empty loop and reports an enormous speedup.

## Problems

Four. The first three are in `tests/ch16/measuring.c` and are graded against definitions the tests
...
```bash
python3 -m pytest tests/ch16/test_problem_1_summary.py
```
...
```bash
python3 -m pytest tests/ch16/test_problem_2_repetitions.py
```
...
```bash
python3 -m pytest tests/ch16/test_problem_3_warmup.py
```
...
as cheap as it is, on Linux, is that it usually is not a system call at all — [ch08](#ch08)'s trap
path is avoided by mapping a page of kernel data into every process, which is [ch09](#ch09)'s
mechanism used for something [ch09](#ch09) had no reason to mention.

[ch17](#ch17) is the first chapter to ask the machine a question, and it is the one this book's
running example has been waiting for: [ch15](#ch15)'s two routes differ because of where the data
