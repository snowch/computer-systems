...
[ch14](#ch14) ended with a walk that stopped and a level to report it at. That was presented as a
...
Here is the difference from [ch13](#ch13) that makes it work, and it is one line of the kernel.
...
```{figure} _figures/ch15-decision.svg
...
```{include} _generated/ch15-exchange.md
```

Read the first four rows as one sentence. Most of what was asked for lazily was never allocated,
and the pages that were allocated each cost one entry into the kernel — the whole of [ch13](#ch13)'s
...
```{include} _generated/ch15-causes.md
```
...
interruption — and this target models none of the last part. [ch26](#ch26) prices the whole shape
...
Three, in `tests/ch15/policy.c`. They are the three decisions the handler is really making, taken
...
```bash
python3 -m pytest tests/ch15/test_problem_1_faults.py
```
...
```bash
python3 -m pytest tests/ch15/test_problem_2_action.py
```
...
```bash
python3 -m pytest tests/ch15/test_problem_3_observed.py
```
...
[ch16](#ch16) stays with the same mechanism and changes what raises it. A page fault is the CPU
