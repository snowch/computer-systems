...
short_title: "ch18 Optimising Code"
---
...
[ch17](#ch17) ended by saying that a memory access costs what it costs because of what else the
...
```{include} _generated/ch18-variants.md
```
...
`tests/ch18/escapes.c` contains five functions doing identical arithmetic. They differ only in
...
counts, which is [ch03](#ch03)'s keyword doing the job it exists for. Storing it somewhere nothing
...
```{include} _generated/ch18-cost.md
```
...
unrolled version is also *slower* is a question about [ch19](#ch19)'s machinery, and the answer is
...
**Whether fewer instructions is faster.** It very often is not, and [ch19](#ch19) is the chapter
...
```bash
python3 -m pytest tests/ch18/test_problem_1_equalised.py
```
...
```bash
python3 -m pytest tests/ch18/test_problem_3_escapes.py
```
...
[ch19](#ch19) is the chapter this one keeps deferring to. Instruction counts cannot say whether
