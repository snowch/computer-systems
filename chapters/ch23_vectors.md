...
| **Prerequisites** | [ch22](#ch22) |
| **Assumes** | a vector unit — NEON on the reference core. This chapter became measurable when Part IV moved to AArch64; on a RISC-V board without RVV 1.0 it reverts to reasoning about code the compiler emits but the hardware cannot run. |
...
```{include} _generated/ch23-loops.md
```
...
```{include} _generated/ch23-scale.md
```
...
```{include} _generated/ch23-sum-f32.md
```
...
```{include} _generated/ch23-running.md
```
...
This is [ch19](#ch19)'s dependence chain again, at a different granularity and with the same
conclusion.
...
```{include} _generated/ch23-speedup.md
```
...
**Whether any of this is where your time is going.** [ch22](#ch22) is the chapter for that, and
...
```bash
python3 -m pytest tests/ch23/test_problem_1_tail.py
```
...
```bash
python3 -m pytest tests/ch23/test_problem_2_reassociation.py
```
...
```bash
python3 -m pytest tests/ch23/test_problem_3_bound.py
```
...
it for the same reason [ch20](#ch20) put two memory models side by side: it solves the tail
