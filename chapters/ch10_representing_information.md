...
```{figure} _figures/ch10-padding.svg
...
which is [ch22](#ch22)'s subject and the first place this dry rule becomes a duration.
...
```{include} _generated/ch10-signed-grows.md
```
...
```{include} _generated/ch10-unsigned-grows.md
```
...
```{include} _generated/ch10-unsigned-quarter.md
```
...
```{include} _generated/ch10-signed-quarter.md
```
...
backwards. [ch16](#ch16) meets the third kind for real, reading a device that does not agree with
...
ordering is the whole of the lesson in [ch21](#ch21) about checking for a condition while the
...
faster than one with fewer, and [ch24](#ch24) shows a case where that happens for reasons
...
needed immediately. [ch23](#ch23) is where that gets a number, on a machine that can produce one.
...
perfectly reasonable thing for a teaching kernel to decide — it makes [ch18](#ch18)'s context
...
`tests/ch10/bitops.c` asks for a byte-order swap and the index of the lowest set bit. Neither is
...
```bash
python3 -m pytest tests/ch10/test_problem_1_bitops.py
```
...
```bash
python3 -m pytest tests/ch10/test_problem_2_reorder.py
```
...
```bash
python3 -m pytest tests/ch10/test_problem_3_break_it.py
```
