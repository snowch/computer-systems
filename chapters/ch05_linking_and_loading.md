...
short_title: "ch05 Linking and Loading"
---
...
[ch01](#ch01) left the linker as the stage that "fills in the holes" and moved on. This chapter
...
```{figure} _figures/ch05-segments.svg
...
```{include} _generated/ch05-shape.md
```
...
```{include} _generated/ch05-segment-table.md
```
...
memory — [ch01](#ch01)'s comparison of glibc against xv6's library was about code, and this is the
...
prohibitions are enforced by the page table rather than by convention. [ch07](#ch07) is where that
enforcement becomes a mechanism you can see and [ch08](#ch08) is where breaking it becomes a fault
you can catch.
...
permanently — which is why [ch20](#ch20) spends time on keeping symbols around for the profiler.
...
[ch19](#ch19) touches the cost of the machinery; this chapter does not describe it.
...
copied — and this chapter has no way to measure it. [ch19](#ch19) measures what a process costs to
...
```bash
python3 -m pytest tests/ch05/test_problem_1_reader.py
```
...
```bash
python3 -m pytest tests/ch05/test_problem_2_resolve.py
```
...
```bash
python3 -m pytest tests/ch05/test_problem_3_diagnose.py
```
...
surprises at all. That feeling is what Part I was for.

[ch06](#ch06) begins Part II by asking what happens when that program asks the kernel for
something.
