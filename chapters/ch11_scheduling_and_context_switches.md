...
short_title: "ch11 Scheduling and Context Switches"
---
...
[ch10](#ch10) ended on something that ought to be impossible: a lock held across a context switch.
...
```{include} _generated/ch11-swtch.md
```

Read the first two rows against each other, because the ratio is the point.

[ch06](#ch06)'s trap path saves thirty-one registers. `swtch` saves fourteen. Both are moving "the
state of a thread" and one of them moves less than half as much, which looks like an optimisation
and is not.

**A trap is not a call and a switch is.** The interrupted program of [ch06](#ch06) agreed to
nothing, so everything it might have been using has to be preserved. `swtch` is reached by an
ordinary `jal` from `sched`, which means [ch04](#ch04)'s calling convention has already been
...
This is also the answer to [ch10](#ch10)'s impossibility. The lock is held by a *process*, and the
...
and `pop_off` count nesting per CPU rather than per process, and why [ch10](#ch10) found them
...
```{include} _generated/ch11-census.md
```
...
depends on whether the thing it waited for had already happened. [ch09](#ch09) established this
...
[ch09](#ch09)'s console driver slept, and this is where that gets settled.
...
caches and the TLB full of somebody else's data. [ch19](#ch19) is where that gets a number, and
...
whole length explaining why that question goes to Part III.
...
Three, in `tests/ch11/scheduling.c`.

**11.1 — Must the switch save this register?**
Given the register's role under [ch04](#ch04)'s convention and whether the value is still needed,
...
```bash
python3 -m pytest tests/ch11/test_problem_1_save.py
```
...
```bash
python3 -m pytest tests/ch11/test_problem_2_wakeup.py
```
...
```bash
python3 -m pytest tests/ch11/test_problem_3_policy.py
```
...
fourteen lines [ch04](#ch04) sent you to look at without explaining. It has not changed; you have.

[ch12](#ch12) is the last of Part II and the one with a disk in it. A file system has to survive
