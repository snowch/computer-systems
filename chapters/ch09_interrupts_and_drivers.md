...
short_title: "ch09 Interrupts and Drivers"
---
...
[ch08](#ch08) was about the CPU interrupting itself. A page fault happens *because of* the
...
device is allowed to speak, and the processor takes exactly the trap path [ch06](#ch06) counted —
...
```{literalinclude} ../xv6/patches/09-interrupt-census.patch
:language: diff
...
```{figure} _figures/ch09-sources.svg
...
```{include} _generated/ch09-cost.md
```
...
This is [ch06](#ch06)'s rule arriving from a new direction. There the uncountable thing was the
...
machine the chapter runs on. [ch19](#ch19) is on hardware where it is not.
...
innocent process that was running — and this target models none of the last part. [ch19](#ch19)
prices it.

**How often the timer really fires.** The census counts timer interrupts and the book does not
print the number, for [ch06](#ch06)'s reason. It is worth knowing that this count is *steady* here
...
path do something has to run this on hardware, which is Part III.
...
settings are the difference between a measurement and a puzzle, and [ch20](#ch20) has to care.

## Problems

Three. The first two are in `tests/ch09/console.c`; the third is a Python stub you fill in with an
...
```bash
python3 -m pytest tests/ch09/test_problem_1_lost.py
```
...
```bash
python3 -m pytest tests/ch09/test_problem_2_interrupts.py
```
...
```bash
python3 -m pytest tests/ch09/test_problem_3_attribute.py
```
...
[ch10](#ch10) is about the problem this chapter has been carefully stepping around. A handler and
