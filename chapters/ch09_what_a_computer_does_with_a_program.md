...
```{figure} _figures/ch09-stages.svg
...
```{include} _generated/ch09-folded.md
```
...
```{include} _generated/ch09-counted.md
```
...
[ch10](#ch10)'s subject, arriving early and uninvited, as it tends to.
...
```{include} _generated/ch09-stage-sizes.md
```
...
```{include} _generated/ch09-linking.md
```
...
identical — it is the cost of everything `printf` is prepared to do if asked. [ch12](#ch12) opens
...
worth listening to, and the board is not attached to this chapter. [ch20](#ch20) puts that
question properly, and [ch23](#ch23) answers it.
...
instructions is not automatically cheaper than one that compiles to ten. [ch22](#ch22) shows a
...
`tests/ch09/problem_1_stages.py` gives you fragments of the four files and asks which program
...
```bash
python3 -m pytest tests/ch09/test_problem_1_stages.py
```
...
```bash
python3 -m pytest tests/ch09/test_problem_2_ripple.py
```
...
```bash
python3 -m pytest tests/ch09/test_problem_3_unfold.py
```
...
reads: sections, symbols and relocations, all of which [ch12](#ch12) takes apart properly. It is
...
[ch10](#ch10) goes back to the thing the compiler was manipulating in the first place: what a
