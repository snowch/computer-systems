...
[ch18](#ch18) ended on a deferral: instruction counts cannot say whether the longer program is the
...
```{include} _generated/ch19-shapes.md
```
...
That is [ch18](#ch18)'s lesson arriving one chapter later and biting this book: what you wrote and
...
one-accumulator version is the **shortest** program. By [ch18](#ch18)'s measure it should be the
best one.
...
```{include} _generated/ch19-ilp.md
```
...
number of those, and past that point they spill to memory and [ch17](#ch17) takes over.
...
```{include} _generated/ch19-branches.md
```
...
The timings and the counter readings are pending. They need the board, and [ch16](#ch16)'s
...
**Whether the compiler will do any of this for you.** [ch18](#ch18) established that it does the
...
```bash
python3 -m pytest tests/ch19/test_problem_1_branchy.py
```
...
```bash
python3 -m pytest tests/ch19/test_problem_2_path.py
```
...
```bash
python3 -m pytest tests/ch19/test_problem_3_predictor.py
```
...
[ch20](#ch20) takes the same machinery and adds a second core. Everything in this chapter assumed
one thread with the whole machine to itself; the next one is about what four of them cost each
other, and it is where [ch12](#ch12)'s fences finally get a price.
