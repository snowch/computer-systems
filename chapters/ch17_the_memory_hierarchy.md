...
short_title: "ch17 The Memory Hierarchy"
---
...
| **Answers the cost of** | [ch05](#ch05), [ch09](#ch09) |
...
[ch15](#ch15) left a prediction on the table: two routes over the same array, differing by one
...
knowing, and the difference matters for a reason [ch00](#ch00) states as policy: a specification
...
```{include} _generated/ch17-levels.md
```
...
```{include} _generated/ch17-line.md
```
...
also, quietly, the answer to a question [ch05](#ch05) raised and could not settle: alignment and
...
[ch09](#ch09) established that every address is a question answered by a walk through three levels
...
```{include} _generated/ch17-vendor.md
```
...
that missed. [ch09](#ch09)'s three levels are three more memory accesses, and this is where that
...
[ch15](#ch15)'s sequential route walks an array in order: every line is used completely before the
...
That is the lesson Part IV exists for, and it is worth stating in the form that transfers: **the
...
touched — each a dependent chase, each reported as a minimum over repetitions for [ch16](#ch16)'s
...
write-allocate policies, and the coherence traffic [ch20](#ch20) is about — and measuring reads
...
predictable access pattern may never see any of them. [ch18](#ch18) is where making a pattern
predictable becomes a thing you do deliberately.

## Problems

Three, in `tests/ch17/hierarchy.c`, graded against curves the tests construct with known answers.
...
```bash
python3 -m pytest tests/ch17/test_problem_1_steps.py
```
...
```bash
python3 -m pytest tests/ch17/test_problem_2_line.py
```
...
```bash
python3 -m pytest tests/ch17/test_problem_3_lines.py
```
...
[ch18](#ch18) is the other half of this. Knowing where the data is tells you what a program costs;
