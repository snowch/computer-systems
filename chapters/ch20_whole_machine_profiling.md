...
short_title: "ch20 Whole-Machine Profiling"
---
...
```{include} _generated/ch20-census.md
```
...
counted in lines, and there are no unvisited lines. This is [ch18](#ch18)'s observation about coherence reappearing as
...
in the source of that function, and [ch17](#ch17) already established what a branch this lopsided
costs a predictor.
...
```{figure} _figures/ch20-sampling.svg
...
[ch09](#ch09)'s mechanism, in hardware you have already met, doing a job that has nothing to do
...
```{include} _generated/ch20-scatter.md
```
...
```{include} _generated/ch20-skid.md
```
...
```{include} _generated/ch20-profile.md
```
...
enough to collect samples. For anything shorter, the equipment is [ch14](#ch14)'s — repetition and
...
Three, in `tests/ch20/profiler.c`.
...
```bash
python3 -m pytest tests/ch20/test_problem_1_inclusive.py
```
...
```bash
python3 -m pytest tests/ch20/test_problem_2_noise.py
```
...
```bash
python3 -m pytest tests/ch20/test_problem_3_aliasing.py
```
...
[ch21](#ch21) is the last measurement in the book and the narrowest: one loop, one unit, and the
