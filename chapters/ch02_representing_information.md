...
short_title: "ch02 Representing Information"
---
...
[ch00](#ch00) measured two structs with the same three members in different orders and found one
larger than the other. A table can say that. It cannot say where the extra bytes went, and that
is the part worth seeing:

```{figure} _figures/ch02-padding.svg
...
which is [ch15](#ch15)'s subject and the first place this dry rule becomes a duration.
...
```{include} _generated/ch02-signed-grows.md
```
...
```{include} _generated/ch02-unsigned-grows.md
```
...
```{include} _generated/ch02-unsigned-quarter.md
```
...
```{include} _generated/ch02-signed-quarter.md
```
...
fact appears in [ch00](#ch00)'s output and not in a footnote here.
...
backwards. [ch09](#ch09) meets the third kind for real, reading a device that does not agree with
...
ordering is the whole of the lesson in [ch14](#ch14) about checking for a condition while the
...
[ch00](#ch00)'s table, measured by booting the kernel and asking it. It is not repeated here,
...
faster than one with fewer, and [ch17](#ch17) shows a case where that happens for reasons
...
needed immediately. [ch16](#ch16) is where that gets a number, on a machine that can produce one.
...
perfectly reasonable thing for a teaching kernel to decide — it makes [ch11](#ch11)'s context
switch small enough to read in one sitting — and it means floating point arrives in Part III,
...
`tests/ch02/bitops.c` asks for a byte-order swap and the index of the lowest set bit. Neither is
...
```bash
python3 -m pytest tests/ch02/test_problem_1_bitops.py
```
...
```bash
python3 -m pytest tests/ch02/test_problem_2_reorder.py
```
...
```bash
python3 -m pytest tests/ch02/test_problem_3_break_it.py
```
...
[ch03](#ch03) takes the other half of C — the part that is really about addresses — and does the
same thing to it.
