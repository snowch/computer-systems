...
short_title: "ch15 The Same Program on Both Targets"
---
...
| **Answers the cost of** | [ch06](#ch06), [ch08](#ch08), [ch09](#ch09) |
...
Here is a program small enough that Parts II and III explain it completely.
...
```{include} _generated/ch15-agreement.md
```
...
different everything else — because [ch05](#ch05) checked the data model matched, and because the
...
```{include} _generated/ch15-sequential.md
```

```{include} _generated/ch15-chased.md
```
...
So: one more load, and a shift and an add. Everything Part II and Part III have to say about these
...
it was arrived at correctly, and Part IV exists because of how wrong it is.

### The number that is not here

```{include} _generated/ch15-cost.md
```
...
both targets are LP64 with the same alignment rules, which [ch05](#ch05) measured rather than
assumed. A book that had chosen a 32-bit target for Part II would have had to say something quite
different here.
...
What changes across the crossing is every number attached to them, which is exactly why Part IV
is arranged as Part III's chapters asked again.
...
The chapter is written so that it reads correctly either way. What Parts II and III predict about
...
second board. [ch16](#ch16) starts from the machine the book does have.
...
Three, in `tests/ch15/crossing.c`. All three are graded against a table of six configurations that
...
```bash
python3 -m pytest tests/ch15/test_problem_1_isolates.py
```
...
```bash
python3 -m pytest tests/ch15/test_problem_2_pair.py
```
...
```bash
python3 -m pytest tests/ch15/test_problem_3_transfers.py
```

## Where to go next

Part IV. Every chapter of it names the earlier chapter whose cost it measures, and the first of
them is about the instrument rather than the machine: before measuring anything, [ch16](#ch16)
