...
```{include} _generated/ch19-amplification.md
```
...
and the disk driver from [ch16](#ch16).
...
That is [ch16](#ch16)'s buffer cache. A block already in memory is not fetched, and a short
...
than to a processor, and shares nothing with [ch17](#ch17)'s fences but a name.
...
Three, in `tests/ch19/filesystem.c`.
...
```bash
python3 -m pytest tests/ch19/test_problem_1_writes.py
```
...
```bash
python3 -m pytest tests/ch19/test_problem_2_crash.py
```
...
```bash
python3 -m pytest tests/ch19/test_problem_3_ordering.py
```
...
[ch20](#ch20) is the hinge. Everything Part IV has established is about what a program *does*, on a
