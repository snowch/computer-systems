...
short_title: "04 · C for People Who Will Read a Kernel"
---

(c-for-people-who-will-read-a-kernel)=
# 04 · C for People Who Will Read a Kernel
...
| **Prerequisites** | [ch03](#c-without-a-runtime) |
...
primitive — [ch19](#locks-and-memory-ordering) is emphatic about that.
...
[ch19](#locks-and-memory-ordering)'s. It means one thing: **every access in the source
...
turned your driver into a program that receives one character forever. [ch18](#interrupts-and-drivers) writes that
driver.
...
buffer, and the reason [ch12](#representing-information)'s bounds-check problem is a real bug pattern rather than an
invention.
...
[ch18](#interrupts-and-drivers) uses this for real. [ch26](#the-cpu) comes back to it with a cost attached, because a
...
moment the function returns. [ch13](#machine-level-code-on-riscv) shows you the frame. A pointer to it, returned, is a
...
by the same machinery [ch11](#what-a-computer-does-with-a-program) introduced and regenerated by CI on each push.
...
[ch26](#the-cpu) measures it; a chapter with no clock in it cannot.
...
sufficient for your problem, and for anything involving two harts it is not. [ch19](#locks-and-memory-ordering) is
...
**4.2 — A function that works until it is called twice.**
...
[ch13](#machine-level-code-on-riscv) stops reading C and starts reading what it became: registers, the calling
