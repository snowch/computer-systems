...
| **Prerequisites** | [ch10](#representing-information) |
...
primitive — [ch17](#locks-and-memory-ordering) is emphatic about that.
...
[ch17](#locks-and-memory-ordering)'s. It means one thing: **every access in the source
...
turned your driver into a program that receives one character forever. [ch16](#interrupts-and-drivers) writes that
driver.
...
buffer, and the reason [ch10](#representing-information)'s bounds-check problem is a real bug pattern rather than an
invention.
...
[ch16](#interrupts-and-drivers) uses this for real. [ch24](#the-cpu) comes back to it with a cost attached, because a
...
moment the function returns. [ch11](#machine-level-code-on-riscv) shows you the frame. A pointer to it, returned, is a
...
by the same machinery [ch09](#what-a-computer-does-with-a-program) introduced and regenerated by CI on each push.
...
[ch24](#the-cpu) measures it; a chapter with no clock in it cannot.
...
sufficient for your problem, and for anything involving two harts it is not. [ch17](#locks-and-memory-ordering) is
...
[ch11](#machine-level-code-on-riscv) stops reading C and starts reading what it became: registers, the calling
