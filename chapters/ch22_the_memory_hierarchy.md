...
| **Answers the cost of** | [ch10](#ch10), [ch14](#ch14) |
| **Prerequisites** | [ch21](#ch21) |
...
[ch20](#ch20) left a prediction on the table: two routes over the same array, differing by one
...
```{include} _generated/ch22-levels.md
```
...
```{include} _generated/ch22-line.md
```
...
also, quietly, the answer to a question [ch10](#ch10) raised and could not settle: alignment and
...
[ch14](#ch14) established that every address is a question answered by a walk through three levels
...
```{include} _generated/ch22-vendor.md
```
...
that missed. [ch14](#ch14)'s three levels are three more memory accesses, and this is where that
...
[ch20](#ch20)'s sequential route walks an array in order: every line is used completely before the
...
touched — each a dependent chase, each reported as a minimum over repetitions for [ch21](#ch21)'s
...
write-allocate policies, and the coherence traffic [ch25](#ch25) is about — and measuring reads
...
predictable access pattern may never see any of them. [ch23](#ch23) is where making a pattern
predictable becomes a thing you do deliberately.

## Problems

Three, in `tests/ch22/hierarchy.c`, graded against curves the tests construct with known answers.
...
```bash
python3 -m pytest tests/ch22/test_problem_1_steps.py
```
...
```bash
python3 -m pytest tests/ch22/test_problem_2_line.py
```
...
```bash
python3 -m pytest tests/ch22/test_problem_3_lines.py
```
...
[ch23](#ch23) is the other half of this. Knowing where the data is tells you what a program costs;
