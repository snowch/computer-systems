...
short_title: "ch13 Traps and System Calls"
---
...
accordingly, and the convention tells both sides who preserves what — that was [ch11](#ch11).
...
```{figure} _figures/ch13-trap-path.svg
...
[ch14](#ch14) is where page tables become a mechanism rather than a word; this is the one place in
Part IV where a chapter has to promise that something later will make sense.

### How long is the path?

```{include} _generated/ch13-path-counts.md
```
...
neither. What is true regardless of the machine is that the path is this long. [ch26](#ch26) takes
...
was easy, impressive, and meaningless. [ch26](#ch26) counts the lot, on a machine where elapsed
...
possible for the *shorter* of two paths to be the slower one, and [ch26](#ch26) is where that gets
settled.
...
about them, because their frequency here is an artefact of emulation. [ch18](#ch18) explains what
...
and what happens when one arrives at the wrong moment, are [ch16](#ch16)'s subject.
...
```bash
python3 -m pytest tests/ch13/test_problem_1_addcall.py
```
...
```bash
python3 -m pytest tests/ch13/test_problem_2_registers.py
```
...
[ch14](#ch14) takes the promise this chapter made — that a page table is a thing, that the
