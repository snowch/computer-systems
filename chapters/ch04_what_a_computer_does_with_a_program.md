...
short_title: "ch04 What a Computer Does With a Program"
---
...
Separating the two is the skill the rest of Part II is built on, because you cannot ask what a
...
```{figure} _figures/ch04-stages.svg
...
```{include} _generated/ch04-folded.md
```
...
```{include} _generated/ch04-counted.md
```
...
[ch05](#ch05)'s subject, arriving early and uninvited, as it tends to.
...
```{include} _generated/ch04-stage-sizes.md
```
...
```{include} _generated/ch04-linking.md
```
...
identical — it is the cost of everything `printf` is prepared to do if asked. [ch07](#ch07) opens
...
worth listening to, and the board is not attached to this chapter. [ch15](#ch15) puts that
question properly, and [ch18](#ch18) answers it.
...
instructions is not automatically cheaper than one that compiles to ten. [ch17](#ch17) shows a
...
`tests/ch04/problem_1_stages.py` gives you fragments of the four files and asks which program
...
```bash
python3 -m pytest tests/ch04/test_problem_1_stages.py
```
...
```bash
python3 -m pytest tests/ch04/test_problem_2_ripple.py
```
...
```bash
python3 -m pytest tests/ch04/test_problem_3_unfold.py
```
...
reads: sections, symbols and relocations, all of which [ch07](#ch07) takes apart properly. It is
...
[ch05](#ch05) goes back to the thing the compiler was manipulating in the first place: what a
