...
| **Answers the cost of** | [ch11](#ch11) |
| **Prerequisites** | [ch23](#ch23) |
| **Assumes** | An out-of-order core (Cortex-A76) and its performance counters. An in-order core would make this easier to read; see [ch00](#ch00). |
...
[ch23](#ch23) ended on a deferral: instruction counts cannot say whether the longer program is the
...
```{include} _generated/ch24-shapes.md
```
...
That is [ch23](#ch23)'s lesson arriving one chapter later and biting this book: what you wrote and
...
one-accumulator version is the **shortest** program. By [ch23](#ch23)'s measure it should be the
best one.
...
```{include} _generated/ch24-ilp.md
```
...
number of those, and past that point they spill to memory and [ch22](#ch22) takes over.
...
```{include} _generated/ch24-branches.md
```
...
The timings and the counter readings are pending. They need the board, and [ch21](#ch21)'s
...
because that is what the reference machine has and because [ch00](#ch00) explains at length why
...
**Whether the compiler will do any of this for you.** [ch23](#ch23) established that it does the
...
```bash
python3 -m pytest tests/ch24/test_problem_1_branchy.py
```
...
```bash
python3 -m pytest tests/ch24/test_problem_2_path.py
```
...
```bash
python3 -m pytest tests/ch24/test_problem_3_predictor.py
```
...
[ch25](#ch25) takes the same machinery and adds a second core. Everything in this chapter assumed
one thread with the whole machine to itself; the next one is about what four of them cost each
other, and it is where [ch17](#ch17)'s fences finally get a price.
