...
[ch16](#ch16) ended on a deferral: instruction counts cannot say whether the longer program is the
...
```{include} _generated/ch17-shapes.md
```
...
That is [ch16](#ch16)'s lesson arriving one chapter later and biting this book: what you wrote and
...
one-accumulator version is the **shortest** program. By [ch16](#ch16)'s measure it should be the
best one.
...
```{include} _generated/ch17-ilp.md
```
...
number of those, and past that point they spill to memory and [ch15](#ch15) takes over.
...
```{include} _generated/ch17-branches.md
```
...
The timings and the counter readings are pending. They need the board, and [ch14](#ch14)'s
...
**Whether the compiler will do any of this for you.** [ch16](#ch16) established that it does the
...
```bash
python3 -m pytest tests/ch17/test_problem_1_branchy.py
```
...
```bash
python3 -m pytest tests/ch17/test_problem_2_path.py
```
...
```bash
python3 -m pytest tests/ch17/test_problem_3_predictor.py
```
...
[ch18](#ch18) takes the same machinery and adds a second core. Everything in this chapter assumed
one thread with the whole machine to itself; the next one is about what four of them cost each
other, and it is where [ch10](#ch10)'s fences finally get a price.
