...
```{figure} _figures/ch11-frame.svg
...
```{include} _generated/ch11-frames.md
```
...
```{include} _generated/ch11-leaf.md
```
...
```{include} _generated/ch11-calls-out.md
```
...
branch that is taken or not — which is why [ch09](#ch09)'s counted loop ended in a branch pointing
...
them. That difference resurfaces in [ch23](#ch23) on a machine that does have one, and it is one
...
reasons [ch22](#ch22) explains and this chapter has no way to demonstrate.
...
[ch24](#ch24) is largely about why it does not.
...
Appendix F is the translation for the reader who meets it in [ch23](#ch23).
...
```bash
python3 -m pytest tests/ch11/test_problem_1_reconstruct.py
```
...
```bash
python3 -m pytest tests/ch11/test_problem_2_frames.py
```
...
```bash
python3 -m pytest tests/ch11/test_problem_3_who_saves.py
```
...
Read it now. You will not know *why* it is called or what a context is until [ch18](#ch18), but
you can already read every instruction in it, which is a good way to find out that you can.

[ch12](#ch12) asks where all this ends up: sections, segments, symbols, and what `exec` does with
them.
