...
[ch13](#ch13) established that the structural model does not predict cost, and pointed at a
...
```{include} _generated/ch14-clock.md
```
...
```{include} _generated/ch14-spread.md
```
...
The first few measurements are slower, always, and for reasons Parts I and II have already
explained: [ch08](#ch08)'s pages are not yet faulted in, [ch15](#ch15)'s caches hold somebody
else's data, and the branch predictor of [ch17](#ch17) has never seen this loop.
...
```{include} _generated/ch14-bias.md
```
...
[ch09](#ch09)'s subject arriving where it is least wanted.
...
answer is to find out what distinguishes them, which is [ch20](#ch20)'s equipment.
...
later uses — is stated rather than demonstrated. [ch16](#ch16) shows what happens when it is
forgotten, which is a benchmark that measures an empty loop and reports an enormous speedup.

## Problems

Four. The first three are in `tests/ch14/measuring.c` and are graded against definitions the tests
...
```bash
python3 -m pytest tests/ch14/test_problem_1_summary.py
```
...
```bash
python3 -m pytest tests/ch14/test_problem_2_repetitions.py
```
...
```bash
python3 -m pytest tests/ch14/test_problem_3_warmup.py
```
...
as cheap as it is, on Linux, is that it usually is not a system call at all — [ch06](#ch06)'s trap
path is avoided by mapping a page of kernel data into every process, which is [ch07](#ch07)'s
mechanism used for something [ch07](#ch07) had no reason to mention.

[ch15](#ch15) is the first chapter to ask the machine a question, and it is the one this book's
running example has been waiting for: [ch13](#ch13)'s two routes differ because of where the data
