...
short_title: "ch07 Linking and Loading"
---
...
[ch04](#ch04) left the linker as the stage that "fills in the holes" and moved on. This chapter
...
```{figure} _figures/ch07-segments.svg
...
```{include} _generated/ch07-segment-table.md
```
...
memory — [ch04](#ch04)'s comparison of glibc against xv6's library was about code, and this is the
...
prohibitions are enforced by the page table rather than by convention. [ch09](#ch09) is where that
enforcement becomes a mechanism you can see and [ch10](#ch10) is where breaking it becomes a fault
you can catch.
...
permanently — which is why [ch22](#ch22) spends time on keeping symbols around for the profiler.
...
[ch21](#ch21) touches the cost of the machinery; this chapter does not describe it.
...
copied — and this chapter has no way to measure it. [ch21](#ch21) measures what a process costs to
...
```bash
python3 -m pytest tests/ch07/test_problem_1_reader.py
```
...
```bash
python3 -m pytest tests/ch07/test_problem_2_resolve.py
```
...
```bash
python3 -m pytest tests/ch07/test_problem_3_diagnose.py
```
...
surprises at all. That feeling is what Part II was for.

[ch08](#ch08) begins Part III by asking what happens when that program asks the kernel for
something.
