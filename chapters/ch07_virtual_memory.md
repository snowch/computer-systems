...
short_title: "ch07 Virtual Memory"
---
...
Every chapter so far has treated an address as a number that names a place. [ch05](#ch05) watched
a loader put a program at the address its linker chose, and [ch06](#ch06) left a promise: the
...
stroke, which is why [ch06](#ch06)'s trap path switches it and why the trampoline has to be mapped
...
```{include} _generated/ch07-sv39.md
```
...
word must all copy bit 38 — a sign extension, exactly as in [ch02](#ch02). An address that fails
...
a stack that has grown one page too far, say, rather than a wild pointer. [ch08](#ch08) is about
...
```{figure} _figures/ch07-address-spaces.svg
...
the program. Two are at the very top: the trapframe and the trampoline, which [ch06](#ch06) put
...
```{include} _generated/ch07-shape.md
```
...
**Those two pages are the trapframe and the trampoline.** The mechanism [ch06](#ch06) measured in
instructions has a second price, in a currency ch06 had no way to express: per process, two pages
of table, for as long as the process exists. Neither number is a cost in the sense this book
usually means — [ch19](#ch19) is where traps get priced in time — but both are real, and the
...
spaces, and [ch06](#ch06) already spent a commit learning what happens when those two get
confused.
...
bookkeeping and tell you about the laptop. [ch15](#ch15) measures what a miss costs on a machine
...
when one of them is violated, is [ch08](#ch08).
...
Three, and they are one program: `tests/ch07/walk.c` builds a page table, follows it, and reports
...
```bash
python3 -m pytest tests/ch07/test_problem_1_map.py
```
...
```bash
python3 -m pytest tests/ch07/test_problem_2_translate.py
```
...
This is the function a page-fault handler needs before it can decide anything, and [ch08](#ch08)
is about the decisions.

```bash
python3 -m pytest tests/ch07/test_problem_3_fault.py
```
...
[ch08](#ch08) takes the failure case seriously. A walk that stops is an opportunity, not an
