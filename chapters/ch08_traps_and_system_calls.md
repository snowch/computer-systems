...
short_title: "ch08 Traps and System Calls"
---
...
Part II ended with a program loaded into an address space and running. It cannot do anything
...
accordingly, and the convention tells both sides who preserves what — that was [ch06](#ch06).
...
```{figure} _figures/ch08-trap-path.svg
...
[ch09](#ch09) is where page tables become a mechanism rather than a word; this is the one place in
Part III where a chapter has to promise that something later will make sense.

### How long is the path?

```{include} _generated/ch08-path-counts.md
```
...
than in Part IV. What an instruction costs depends on a pipeline and a cache, and this target has
neither. What is true regardless of the machine is that the path is this long. [ch21](#ch21) takes
...
```{literalinclude} ../xv6/patches/08-trap-census.patch
:language: diff
...
```{include} _generated/ch08-census.md
```
...
was easy, impressive, and meaningless. [ch21](#ch21) counts the lot, on a machine where elapsed
...
possible for the *shorter* of two paths to be the slower one, and [ch21](#ch21) is where that gets
settled.
...
about them, because their frequency here is an artefact of emulation. [ch13](#ch13) explains what
...
and what happens when one arrives at the wrong moment, are [ch11](#ch11)'s subject.
...
```bash
python3 -m pytest tests/ch08/test_problem_1_addcall.py
```
...
```bash
python3 -m pytest tests/ch08/test_problem_2_registers.py
```
...
[ch09](#ch09) takes the promise this chapter made — that a page table is a thing, that the
