...
[ch16](#ch16) ended owing an answer. Its census counters are incremented from interrupt handlers
...
```{include} _generated/ch17-race.md
```
...
```{include} _generated/ch17-atomic.md
```
...
[ch23](#ch23) is where being told matters.
...
```{include} _generated/ch17-ordered.md
```
...
```{include} _generated/ch17-publish.md
```
...
```{include} _generated/ch17-primitives.md
```
...
[ch16](#ch16)'s counters are incremented without a lock from several harts and from interrupt
...
matter — the ones in [ch15](#ch15) and [ch16](#ch16) — are the ones fixed by the workload rather
...
not deterministic either: [ch16](#ch16) had already found the console's interrupt count varying
...
Both are questions about hardware, and [ch25](#ch25) asks them on a machine that can answer.
...
[ch16](#ch16)'s console driver used, and they are built on top of the spinlock plus the scheduler
rather than beside it. [ch18](#ch18) has the scheduler and can say what sleeping actually means.

## Problems

Three, in `tests/ch17/locking.c`. The first is the only one in this book that is graded by running
...
```bash
python3 -m pytest tests/ch17/test_problem_1_lock.py
```
...
```bash
python3 -m pytest tests/ch17/test_problem_2_reorder.py
```
...
```bash
python3 -m pytest tests/ch17/test_problem_3_order.py
```
...
[ch18](#ch18) is the other half of what the console driver did. A process that cannot get on
