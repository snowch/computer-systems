...
[ch22](#ch22) ended by saying that a memory access costs what it costs because of what else the
...
```{include} _generated/ch23-variants.md
```
...
`tests/ch23/escapes.c` contains five functions doing identical arithmetic. They differ only in
...
```{include} _generated/ch23-cost.md
```
...
unrolled version is also *slower* is a question about [ch24](#ch24)'s machinery, and the answer is
...
**Whether fewer instructions is faster.** It very often is not, and [ch24](#ch24) is the chapter
...
```bash
python3 -m pytest tests/ch23/test_problem_1_equalised.py
```
...
```bash
python3 -m pytest tests/ch23/test_problem_3_escapes.py
```
...
[ch24](#ch24) is the chapter this one keeps deferring to. Instruction counts cannot say whether
