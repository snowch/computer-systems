...
short_title: "ch08 Page Faults as a Feature"
---
...
[ch07](#ch07) ended with a walk that stopped and a level to report it at. That was presented as a
...
Here is the difference from [ch06](#ch06) that makes it work, and it is one line of the kernel.
...
```{figure} _figures/ch08-decision.svg
...
```{literalinclude} ../xv6/patches/08-fault-census.patch
:language: diff
...
```{include} _generated/ch08-exchange.md
```

Read the first four rows as one sentence. Most of what was asked for lazily was never allocated,
and the pages that were allocated each cost one entry into the kernel — the whole of [ch06](#ch06)'s
...
```{include} _generated/ch08-causes.md
```
...
interruption — and this target models none of the last part. [ch19](#ch19) prices the whole shape
...
Three, in `tests/ch08/policy.c`. They are the three decisions the handler is really making, taken
...
```bash
python3 -m pytest tests/ch08/test_problem_1_faults.py
```
...
```bash
python3 -m pytest tests/ch08/test_problem_2_action.py
```
...
```bash
python3 -m pytest tests/ch08/test_problem_3_observed.py
```
...
[ch09](#ch09) stays with the same mechanism and changes what raises it. A page fault is the CPU
