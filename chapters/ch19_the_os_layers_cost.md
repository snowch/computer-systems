...
short_title: "ch19 The OS Layer's Cost on Real Hardware"
---
...
Part II took three things apart — a system call, a page fault, a context switch — and counted
...
```{include} _generated/ch19-model.md
```
...
```{include} _generated/ch19-trap.md
```
...
by [ch04](#ch04)'s rule a function that calls nothing needs none — and this function does not
...
```{include} _generated/ch19-call.md
```
...
[ch14](#ch14) built the clock and, more usefully, measured what reading it costs. This is the
...
```{include} _generated/ch19-cost.md
```
...
```{include} _generated/ch19-faults.md
```

[ch08](#ch08) counted faults and made the case that a fault is a feature — the mechanism by which
...
Both faults in that table enter the kernel by exactly the path [ch06](#ch06) traced, and leave it
...
```{include} _generated/ch19-vdso.md
```
...
This closes a loop opened five chapters earlier. [ch14](#ch14)'s clock is cheap enough to time
things with *because* of this mechanism; a clock that trapped would be an instrument of the same
order as much of what Part III measures, and most of this book's timings would be impossible to
...
saves. The counts they will be compared against are already in the book, taken in Part II, which
is the point of having spent Part II counting.
...
next rather than to the call. Finding that in a real program is [ch20](#ch20)'s equipment.
...
**What xv6 charges.** Part II's counts stay counts. There is no version of this chapter that
...
If the measurement is far above it, this chapter has established that Part II's account is
incomplete without establishing what is missing — and the instruction count is not where the
answer will be found.

## Problems

Three, in `tests/ch19/oscost.c`.
...
```bash
python3 -m pytest tests/ch19/test_problem_1_instrument.py
```

**19.2 — What can it not be cheaper than?**
Turn [ch06](#ch06)'s instruction count into a floor, given an IPC and a clock. Divide once at the
end: a bound that has been rounded twice is an estimate.

```bash
python3 -m pytest tests/ch19/test_problem_2_bound.py
```
...
```bash
python3 -m pytest tests/ch19/test_problem_3_faults.py
```

## Where to go next

The AArch64 exception model is specified in the ARM architecture reference manual, and reading
its description of what `svc` does beside [ch06](#ch06)'s account of `ecall` is the fastest way to
...
[ch20](#ch20) stops assuming you know which code to look at. Everything measured so far has been
