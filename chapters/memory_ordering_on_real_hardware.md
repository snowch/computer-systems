...
short_title: "27 · Memory Ordering on Real Hardware"
---

(memory-ordering-on-real-hardware)=
# 27 · Memory Ordering on Real Hardware
...
| **Answers the cost of** | [ch19](#locks-and-memory-ordering) |
| **Prerequisites** | [ch26](#the-cpu) |
...
[ch19](#locks-and-memory-ordering) established what a lock is made of and said plainly that what it *costs* is
...
Problem 27.1 is the analysis: given where the fields are and who writes them, which pairs will
...
[ch19](#locks-and-memory-ordering) printed what a release looks like on both architectures, and the two are not alike.
...
Problem 27.3 is that correspondence as a table, and the table is not the point. The asymmetry is.
...
[ch23](#measuring) asked for a distribution instead of a number. This chapter asks for something else
first: a **prediction**.
...
coherence traffic are exactly the things that live in the gap. Problem 27.2 is the prediction; the
...
[ch29](#whole-machine-profiling)'s equipment.
...
that relies on not seeing it is broken on a chip that does. [ch19](#locks-and-memory-ordering)'s problem 19.2 is about
...
**27.1 — Which fields will two cores fight over?**
...
**27.2 — What does the curve look like before you measure it?**
...
**27.3 — Same requirement, two architectures.**
...
[ch28](#the-os-layers-cost) returns to [Part IV](#part4) with the same equipment. Everything xv6 demonstrated structurally
