...
short_title: "14 · Machine-Level Code on RISC-V"
---

(machine-level-code-on-riscv)=
# 14 · Machine-Level Code on RISC-V
...
| **Prerequisites** | [ch05](#c-for-people-who-will-read-a-kernel) |
...
```{figure} _figures/machine-level-code-on-riscv-frame.svg
...
`framewalk.c` marks all three of its call-chain functions `noinline`, and the comment saying why
is a lesson rather than a workaround:
...
There is no general answer. Four functions, compiled twice:

```{include} _generated/machine-level-code-on-riscv-frames.md
```
...
multiplications are the same multiplications. **It is keeping values in registers instead of in
...
```{include} _generated/machine-level-code-on-riscv-leaf.md
```
...
branch that is taken or not — which is why [ch12](#what-a-computer-does-with-a-program)'s counted loop ended in a branch pointing
...
Everything above can also be done interactively against a real xv6 binary, where you can step the
instructions rather than read them:
...
The table establishes one thing: **whether a function needs a frame is a property of what it
does, not of how big it looks.** Two of these four shed their frames entirely under optimisation
and two did not, and the two that did not are exactly the two that call something. That is a rule
you can apply to a function you have never seen.
...
reasons [ch25](#the-memory-hierarchy) explains and this chapter has no way to demonstrate.
...
[ch27](#the-cpu) is largely about why it does not.
...
**14.1 — Write the C that produced this.**
...
asking why *that* instruction and not the obvious one, which is the question this chapter has
been teaching you to ask.

```bash
python3 -m pytest tests/machine_level_code_on_riscv/test_problem_1_reconstruct.py
```

**14.2 — Which of these needs a frame?**
...
```bash
python3 -m pytest tests/machine_level_code_on_riscv/test_problem_3_who_saves.py
```
