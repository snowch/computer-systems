...
short_title: "ch12 The File System"
---
...
Every mechanism in Part II so far has assumed the machine keeps running. A lock is held until it
...
```{include} _generated/ch12-amplification.md
```
...
```{include} _generated/ch12-cost.md
```
...
and the disk driver from [ch09](#ch09).
...
That is [ch09](#ch09)'s buffer cache. A block already in memory is not fetched, and a short
...
than to a processor, and shares nothing with [ch10](#ch10)'s fences but a name.
...
Three, in `tests/ch12/filesystem.c`.
...
```bash
python3 -m pytest tests/ch12/test_problem_1_writes.py
```
...
```bash
python3 -m pytest tests/ch12/test_problem_2_crash.py
```
...
```bash
python3 -m pytest tests/ch12/test_problem_3_ordering.py
```
...
[ch13](#ch13) is the hinge. Everything Part II has established is about what a program *does*, on a
...
whole of Part III exists to answer.
