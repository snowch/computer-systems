...
primitive — [ch10](#ch10) is emphatic about that. It means one thing: **every access in the source
...
turned your driver into a program that receives one character forever. [ch09](#ch09) writes that
driver.
...
buffer, and the reason [ch02](#ch02)'s bounds-check problem is a real bug pattern rather than an
invention.
...
[ch09](#ch09) uses this for real. [ch17](#ch17) comes back to it with a cost attached, because a
...
moment the function returns. [ch04](#ch04) shows you the frame. A pointer to it, returned, is a
...
by the same machinery [ch01](#ch01) introduced and regenerated by CI on each push.
...
[ch17](#ch17) measures it; a chapter with no clock in it cannot.
...
sufficient for your problem, and for anything involving two harts it is not. [ch10](#ch10) is
...
[ch04](#ch04) stops reading C and starts reading what it became: registers, the calling
