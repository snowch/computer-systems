...
short_title: "ch03 C for People Who Will Read a Kernel"
---
...
```{include} _generated/ch03-plain-reads.md
```
...
[ch17](#ch17)'s. It means one thing: **every access in the source
...
turned your driver into a program that receives one character forever. [ch16](#ch16) writes that
driver.
...
```{include} _generated/ch03-array-parameter.md
```
...
buffer, and the reason [ch10](#ch10)'s bounds-check problem is a real bug pattern rather than an
invention.
...
```{include} _generated/ch03-private-call.md
```
...
```{include} _generated/ch03-indirect-call.md
```
...
```{figure} _figures/ch03-dispatch.svg
...
[ch16](#ch16) uses this for real. [ch24](#ch24) comes back to it with a cost attached, because a
...
moment the function returns. [ch11](#ch11) shows you the frame. A pointer to it, returned, is a
...
by the same machinery [ch09](#ch09) introduced and regenerated by CI on each push.
...
[ch24](#ch24) measures it; a chapter with no clock in it cannot.
...
sufficient for your problem, and for anything involving two harts it is not. [ch17](#ch17) is
...
```bash
python3 -m pytest tests/ch03/test_problem_1_which_c.py
```

**3.2 — A function that works until it is called twice.**
`tests/ch03/scratchpad.c` turns a number into text and is correct in isolation. Called twice
...
```bash
python3 -m pytest tests/ch03/test_problem_2_scratchpad.py
```
...
results. Read what the functions in `tests/ch03/devices.c` *do* rather than what they are called,
...
```bash
python3 -m pytest tests/ch03/test_problem_3_dispatch.py
```
...
[ch11](#ch11) stops reading C and starts reading what it became: registers, the calling
