...
turned your driver into a program that receives one character forever. [ch11](#ch11) writes that
driver.
...
buffer, and the reason [ch05](#ch05)'s bounds-check problem is a real bug pattern rather than an
invention.
...
[ch11](#ch11) uses this for real. [ch19](#ch19) comes back to it with a cost attached, because a
...
moment the function returns. [ch06](#ch06) shows you the frame. A pointer to it, returned, is a
...
by the same machinery [ch04](#ch04) introduced and regenerated by CI on each push.
...
[ch19](#ch19) measures it; a chapter with no clock in it cannot.
...
sufficient for your problem, and for anything involving two harts it is not. [ch12](#ch12) is
...
[ch06](#ch06) stops reading C and starts reading what it became: registers, the calling
