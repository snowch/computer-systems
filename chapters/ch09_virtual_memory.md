...
short_title: "ch09 Virtual Memory"
---
...
Every chapter so far has treated an address as a number that names a place. [ch07](#ch07) watched
a loader put a program at the address its linker chose, and [ch08](#ch08) left a promise: the
...
stroke, which is why [ch08](#ch08)'s trap path switches it and why the trampoline has to be mapped
...
```{include} _generated/ch09-sv39.md
```
...
word must all copy bit 38 — a sign extension, exactly as in [ch05](#ch05). An address that fails
...
a stack that has grown one page too far, say, rather than a wild pointer. [ch10](#ch10) is about
...
```{figure} _figures/ch09-address-spaces.svg
...
the program. Two are at the very top: the trapframe and the trampoline, which [ch08](#ch08) put
...
```{include} _generated/ch09-shape.md
```
...
**Those two pages are the trapframe and the trampoline.** The mechanism [ch08](#ch08) measured in
instructions has a second price, in a currency ch08 had no way to express: per process, two pages
of table, for as long as the process exists. Neither number is a cost in the sense this book
usually means — [ch21](#ch21) is where traps get priced in time — but both are real, and the
...
spaces, and [ch08](#ch08) already spent a commit learning what happens when those two get
confused.
...
bookkeeping and tell you about the laptop. [ch17](#ch17) measures what a miss costs on a machine
...
when one of them is violated, is [ch10](#ch10).
...
Three, and they are one program: `tests/ch09/walk.c` builds a page table, follows it, and reports
...
```bash
python3 -m pytest tests/ch09/test_problem_1_map.py
```
...
```bash
python3 -m pytest tests/ch09/test_problem_2_translate.py
```
...
This is the function a page-fault handler needs before it can decide anything, and [ch10](#ch10)
is about the decisions.

```bash
python3 -m pytest tests/ch09/test_problem_3_fault.py
```
...
[ch10](#ch10) takes the failure case seriously. A walk that stops is an opportunity, not an
