...
Every chapter so far has treated an address as a number that names a place. [ch12](#ch12) watched
a loader put a program at the address its linker chose, and [ch13](#ch13) left a promise: the
...
stroke, which is why [ch13](#ch13)'s trap path switches it and why the trampoline has to be mapped
...
```{include} _generated/ch14-sv39.md
```
...
word must all copy bit 38 — a sign extension, exactly as in [ch10](#ch10). An address that fails
...
a stack that has grown one page too far, say, rather than a wild pointer. [ch15](#ch15) is about
...
```{figure} _figures/ch14-address-spaces.svg
...
the program. Two are at the very top: the trapframe and the trampoline, which [ch13](#ch13) put
...
```{include} _generated/ch14-shape.md
```
...
**Those two pages are the trapframe and the trampoline.** The mechanism [ch13](#ch13) measured in
...
usually means — [ch26](#ch26) is where traps get priced in time — but both are real, and the
...
spaces, and [ch13](#ch13) already spent a commit learning what happens when those two get
confused.
...
bookkeeping and tell you about the laptop. [ch22](#ch22) measures what a miss costs on a machine
...
when one of them is violated, is [ch15](#ch15).
...
Three, and they are one program: `tests/ch14/walk.c` builds a page table, follows it, and reports
...
```bash
python3 -m pytest tests/ch14/test_problem_1_map.py
```
...
```bash
python3 -m pytest tests/ch14/test_problem_2_translate.py
```
...
This is the function a page-fault handler needs before it can decide anything, and [ch15](#ch15)
is about the decisions.

```bash
python3 -m pytest tests/ch14/test_problem_3_fault.py
```
...
[ch15](#ch15) takes the failure case seriously. A walk that stops is an opportunity, not an
