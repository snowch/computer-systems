...
[ch15](#page-faults-as-a-feature) is the chapter that measures what it costs.
...
What that costs is [ch17](#locks-and-memory-ordering)'s subject and it is not small. What matters here is the habit:
...
is [ch15](#page-faults-as-a-feature)'s mechanism used for something other than memory.
...
C will not stop you. Neither will the hardware, until [ch15](#page-faults-as-a-feature). The three that cost the most
...
- **A pointer to a local, after the function returned.** The bytes are still there and still
  ...
  works in testing.
- **Reading or writing past the end of an array.** Nothing checks. [ch10](#representing-information) has the arithmetic
...
- **Two harts writing one variable with no lock**, which works perfectly until the machine is
  busy. [ch17](#locks-and-memory-ordering) is the chapter, and [ch25](#memory-ordering-on-real-hardware) is what it costs on real hardware.
...
**What any of it costs.** No clock in this part. The free list is [ch15](#page-faults-as-a-feature), locks are
[ch17](#locks-and-memory-ordering) and [ch25](#memory-ordering-on-real-hardware), and the context switch that declines to save floating-point
registers is [ch18](#scheduling-and-context-switches).
