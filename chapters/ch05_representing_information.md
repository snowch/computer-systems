...
short_title: "ch05 Representing Information"
---
...
```{figure} _figures/ch05-padding.svg
...
which is [ch17](#ch17)'s subject and the first place this dry rule becomes a duration.
...
```{include} _generated/ch05-signed-grows.md
```
...
```{include} _generated/ch05-unsigned-grows.md
```
...
```{include} _generated/ch05-unsigned-quarter.md
```
...
```{include} _generated/ch05-signed-quarter.md
```
...
backwards. [ch11](#ch11) meets the third kind for real, reading a device that does not agree with
...
ordering is the whole of the lesson in [ch16](#ch16) about checking for a condition while the
...
faster than one with fewer, and [ch19](#ch19) shows a case where that happens for reasons
...
needed immediately. [ch18](#ch18) is where that gets a number, on a machine that can produce one.
...
perfectly reasonable thing for a teaching kernel to decide — it makes [ch13](#ch13)'s context
switch small enough to read in one sitting — and it means floating point arrives in Part IV,
...
`tests/ch05/bitops.c` asks for a byte-order swap and the index of the lowest set bit. Neither is
...
```bash
python3 -m pytest tests/ch05/test_problem_1_bitops.py
```
...
```bash
python3 -m pytest tests/ch05/test_problem_2_reorder.py
```
...
```bash
python3 -m pytest tests/ch05/test_problem_3_break_it.py
```
