...
Here is a program small enough that Parts I and II explain it completely.
...
```{include} _generated/ch13-agreement.md
```
...
different everything else — because [ch02](#ch02) checked the data model matched, and because the
...
```{include} _generated/ch13-sequential.md
```

```{include} _generated/ch13-chased.md
```
...
So: one more load, and a shift and an add. Everything Part I and Part II have to say about these
...
it was arrived at correctly, and Part III exists because of how wrong it is.

### The number that is not here

```{include} _generated/ch13-cost.md
```
...
both targets are LP64 with the same alignment rules, which [ch02](#ch02) measured rather than
assumed. A book that had chosen a 32-bit target for Part I would have had to say something quite
different here.
...
What changes across the crossing is every number attached to them, which is exactly why Part III
is arranged as Part II's chapters asked again.
...
The chapter is written so that it reads correctly either way. What Parts I and II predict about
...
second board. [ch14](#ch14) starts from the machine the book does have.
...
Three, in `tests/ch13/crossing.c`. All three are graded against a table of six configurations that
...
```bash
python3 -m pytest tests/ch13/test_problem_1_isolates.py
```
...
```bash
python3 -m pytest tests/ch13/test_problem_2_pair.py
```
...
```bash
python3 -m pytest tests/ch13/test_problem_3_transfers.py
```

## Where to go next

Part III. Every chapter of it names the earlier chapter whose cost it measures, and the first of
them is about the instrument rather than the machine: before measuring anything, [ch14](#ch14)
