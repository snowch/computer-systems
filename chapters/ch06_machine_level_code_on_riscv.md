...
short_title: "ch06 Machine-Level Code on RISC-V"
---
...
```{figure} _figures/ch06-frame.svg
...
```{include} _generated/ch06-frames.md
```
...
```{include} _generated/ch06-leaf.md
```
...
```{include} _generated/ch06-calls-out.md
```
...
branch that is taken or not — which is why [ch04](#ch04)'s counted loop ended in a branch pointing
...
them. That difference resurfaces in [ch18](#ch18) on a machine that does have one, and it is one
...
reasons [ch17](#ch17) explains and this chapter has no way to demonstrate.
...
[ch19](#ch19) is largely about why it does not.
...
Appendix F is the translation for the reader who meets it in [ch18](#ch18).
...
Three, and the first is the hardest thing in Part II so far.
...
```bash
python3 -m pytest tests/ch06/test_problem_1_reconstruct.py
```
...
```bash
python3 -m pytest tests/ch06/test_problem_2_frames.py
```
...
```bash
python3 -m pytest tests/ch06/test_problem_3_who_saves.py
```
...
Read it now. You will not know *why* it is called or what a context is until [ch13](#ch13), but
you can already read every instruction in it, which is a good way to find out that you can.

[ch07](#ch07) asks where all this ends up: sections, segments, symbols, and what `exec` does with
them.
