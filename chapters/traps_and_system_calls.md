...
| **Prerequisites** | [ch12](#linking-and-loading) |
...
accordingly, and the convention tells both sides who preserves what — that was [ch11](#machine-level-code-on-riscv).
...
[ch14](#virtual-memory) is where page tables become a mechanism rather than a word; this is the one place in
...
neither. What is true regardless of the machine is that the path is this long. [ch26](#the-os-layers-cost) takes
...
was easy, impressive, and meaningless. [ch26](#the-os-layers-cost) counts the lot, on a machine where elapsed
...
possible for the *shorter* of two paths to be the slower one, and [ch26](#the-os-layers-cost) is where that gets
settled.
...
about them, because their frequency here is an artefact of emulation. [ch18](#scheduling-and-context-switches) explains what
...
and what happens when one arrives at the wrong moment, are [ch16](#interrupts-and-drivers)'s subject.
...
[ch14](#virtual-memory) takes the promise this chapter made — that a page table is a thing, that the
