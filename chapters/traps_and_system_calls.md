...
| **Prerequisites** | [ch13](#linking-and-loading) |
...
accordingly, and the convention tells both sides who preserves what — that was [ch12](#machine-level-code-on-riscv).
...
was easy, impressive, and meaningless. [ch27](#the-os-layers-cost) counts the lot, on a machine where elapsed
...
possible for the *shorter* of two paths to be the slower one, and [ch27](#the-os-layers-cost) is where that gets
settled.
...
about them, because their frequency here is an artefact of emulation. [ch19](#scheduling-and-context-switches) explains what
...
and what happens when one arrives at the wrong moment, are [ch17](#interrupts-and-drivers)'s subject.
...
[ch15](#virtual-memory) takes the promise this chapter made — that a page table is a thing, that the
