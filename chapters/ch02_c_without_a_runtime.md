...
```{include} _generated/ch02-absences.md
```
...
```{include} _generated/ch02-pools.md
```
...
combined into the last. `volatile` is how you withdraw those assumptions, and [ch03](#ch03) shows
...
tree and they are written in exactly the style [ch01](#ch01)'s second problem asked for — pointers
...
```bash
python3 -m pytest tests/ch02/test_problem_1_pool.py
```
...
```bash
python3 -m pytest tests/ch02/test_problem_2_failure.py
```
...
```bash
python3 -m pytest tests/ch02/test_problem_3_volatile.py
```
...
[ch03](#ch03) is the last chapter of this part and the one that puts a compiler behind the claims.
