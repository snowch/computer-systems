...
| **Answers the cost of** | [ch13](#ch13), [ch15](#ch15), [ch18](#ch18) |
| **Prerequisites** | [ch25](#ch25) |
...
```{include} _generated/ch26-model.md
```
...
```{include} _generated/ch26-trap.md
```
...
by [ch11](#ch11)'s rule a function that calls nothing needs none — and this function does not
...
```{include} _generated/ch26-call.md
```
...
[ch21](#ch21) built the clock and, more usefully, measured what reading it costs. This is the
...
```{include} _generated/ch26-cost.md
```
...
```{include} _generated/ch26-faults.md
```

[ch15](#ch15) counted faults and made the case that a fault is a feature — the mechanism by which
...
Both faults in that table enter the kernel by exactly the path [ch13](#ch13) traced, and leave it
...
```{include} _generated/ch26-vdso.md
```
...
This closes a loop opened five chapters earlier. [ch21](#ch21)'s clock is cheap enough to time
...
next rather than to the call. Finding that in a real program is [ch27](#ch27)'s equipment.
...
```bash
python3 -m pytest tests/ch26/test_problem_1_instrument.py
```

**19.2 — What can it not be cheaper than?**
Turn [ch13](#ch13)'s instruction count into a floor, given an IPC and a clock. Divide once at the
end: a bound that has been rounded twice is an estimate.

```bash
python3 -m pytest tests/ch26/test_problem_2_bound.py
```
...
```bash
python3 -m pytest tests/ch26/test_problem_3_faults.py
```

## Where to go next

The AArch64 exception model is specified in the ARM architecture reference manual, and reading
its description of what `svc` does beside [ch13](#ch13)'s account of `ecall` is the fastest way to
...
[ch27](#ch27) stops assuming you know which code to look at. Everything measured so far has been
