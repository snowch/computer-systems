...
short_title: "13 · Representing Information"
---

(representing-information)=
# 13 · Representing Information
...
| **Prerequisites** | [ch12](#what-a-computer-does-with-a-program) |
...
which is [ch25](#the-memory-hierarchy)'s subject and the first place this dry rule becomes a duration.
...
**Neither of these functions is wrong, and neither compiler decision is a bug.** The machine code
you get is a consequence of the type you chose, and the type is a claim about what values are
possible. Make a weaker claim and you get more instructions; make a stronger one and the optimiser
will use it, including in ways you did not intend.
...
One function in that file is about overflow rather than bits:
...
ordering is the same lesson [ch24](#measuring) draws about checking for a condition while the
evidence still exists. It arrives several chapters early because arithmetic is where it bites
first.
...
level, regenerated by CI on every push so that a compiler which changes its mind breaks the build
...
needed immediately. [ch26](#optimising-code) is where that gets a number, on a machine that can produce one.
...
**13.1 — Two operations the library does not have.**
...
**13.3 — Find the input that makes it wrong.**
