...
```{include} _generated/ch21-loops.md
```
...
```{include} _generated/ch21-scale.md
```
...
```{include} _generated/ch21-sum-f32.md
```
...
```{include} _generated/ch21-running.md
```
...
This is [ch17](#ch17)'s dependence chain again, at a different granularity and with the same
conclusion.
...
```{include} _generated/ch21-speedup.md
```
...
**Whether any of this is where your time is going.** [ch20](#ch20) is the chapter for that, and
...
```bash
python3 -m pytest tests/ch21/test_problem_1_tail.py
```
...
```bash
python3 -m pytest tests/ch21/test_problem_2_reassociation.py
```
...
```bash
python3 -m pytest tests/ch21/test_problem_3_bound.py
```
...
it for the same reason [ch18](#ch18) put two memory models side by side: it solves the tail
