...
| **Answers the cost of** | [ch11](#ch11), [ch13](#ch13), [ch14](#ch14) |
...
```{include} _generated/ch20-agreement.md
```
...
different everything else — because [ch10](#ch10) checked the data model matched, and because the
...
```{include} _generated/ch20-sequential.md
```

```{include} _generated/ch20-chased.md
```
...
```{include} _generated/ch20-cost.md
```
...
both targets are LP64 with the same alignment rules, which [ch10](#ch10) measured rather than
...
second board. [ch21](#ch21) starts from the machine the book does have.
...
Three, in `tests/ch20/crossing.c`. All three are graded against a table of six configurations that
...
```bash
python3 -m pytest tests/ch20/test_problem_1_isolates.py
```
...
```bash
python3 -m pytest tests/ch20/test_problem_2_pair.py
```
...
```bash
python3 -m pytest tests/ch20/test_problem_3_transfers.py
```

## Where to go next

Part V. Every chapter of it names the earlier chapter whose cost it measures, and the first of
them is about the instrument rather than the machine: before measuring anything, [ch21](#ch21)
