...
short_title: "ch11 Interrupts and Drivers"
---
...
[ch10](#ch10) was about the CPU interrupting itself. A page fault happens *because of* the
...
device is allowed to speak, and the processor takes exactly the trap path [ch08](#ch08) counted —
...
```{literalinclude} ../xv6/patches/11-interrupt-census.patch
:language: diff
...
```{figure} _figures/ch11-sources.svg
...
```{include} _generated/ch11-cost.md
```
...
This is [ch08](#ch08)'s rule arriving from a new direction. There the uncountable thing was the
...
machine the chapter runs on. [ch21](#ch21) is on hardware where it is not.
...
innocent process that was running — and this target models none of the last part. [ch21](#ch21)
prices it.

**How often the timer really fires.** The census counts timer interrupts and the book does not
print the number, for [ch08](#ch08)'s reason. It is worth knowing that this count is *steady* here
...
path do something has to run this on hardware, which is Part IV.
...
settings are the difference between a measurement and a puzzle, and [ch22](#ch22) has to care.

## Problems

Three. The first two are in `tests/ch11/console.c`; the third is a Python stub you fill in with an
...
```bash
python3 -m pytest tests/ch11/test_problem_1_lost.py
```
...
```bash
python3 -m pytest tests/ch11/test_problem_2_interrupts.py
```
...
```bash
python3 -m pytest tests/ch11/test_problem_3_attribute.py
```
...
[ch12](#ch12) is about the problem this chapter has been carefully stepping around. A handler and
