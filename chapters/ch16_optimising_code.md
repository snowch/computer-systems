...
short_title: "ch16 Optimising Code"
---
...
[ch15](#ch15) ended by saying that a memory access costs what it costs because of what else the
...
```{include} _generated/ch16-variants.md
```
...
`tests/ch16/escapes.c` contains five functions doing identical arithmetic. They differ only in
...
unrolled version is also *slower* is a question about [ch17](#ch17)'s machinery, and the answer is
...
**Whether fewer instructions is faster.** It very often is not, and [ch17](#ch17) is the chapter
...
```bash
python3 -m pytest tests/ch16/test_problem_1_equalised.py
```
...
```bash
python3 -m pytest tests/ch16/test_problem_3_escapes.py
```
...
[ch17](#ch17) is the chapter this one keeps deferring to. Instruction counts cannot say whether
