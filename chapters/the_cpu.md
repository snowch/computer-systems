...
| **Answers the cost of** | [ch13](#machine-level-code-on-riscv) |
| **Prerequisites** | [ch25](#optimising-code) |
...
[ch25](#optimising-code) ended on a deferral: instruction counts cannot say whether the longer program is the
...
That is [ch25](#optimising-code)'s lesson arriving one chapter later and biting this book: what you wrote and
...
Problem 26.1 asks you to predict which of the two keeps its branch, before compiling. The stub
...
one-accumulator version is the **shortest** program. By [ch25](#optimising-code)'s measure it should be the
best one.
...
Problem 26.2 is the arithmetic: the critical path is each accumulator's share of the elements plus
...
number of those, and past that point they spill to memory and [ch24](#the-memory-hierarchy) takes over.
...
The predictor itself is a small saturating counter per branch, and problem 26.3 is simulating one.
...
The timings and the counter readings are pending. They need the board, and [ch23](#measuring)'s
...
**Whether the compiler will do any of this for you.** [ch25](#optimising-code) established that it does the
...
**26.1 — Which of these loops still contains a branch?**
...
**26.2 — How long is the critical path?**
...
**26.3 — How many does a two-bit predictor get wrong?**
...
[ch27](#memory-ordering-on-real-hardware) takes the same machinery and adds a second core. Everything in this chapter assumed
one thread with the whole machine to itself; the next one is about what four of them cost each
other, and it is where [ch19](#locks-and-memory-ordering)'s fences finally get a price.
