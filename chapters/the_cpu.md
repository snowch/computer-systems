...
| **What it measures** | What the core is given to work with: `bench/results/pipeline-shapes.json`; and what it does with it: `bench/results/pipeline-host.json` |
:::
...
addition needs the result of the one before it — a *dependency chain*, each step waiting on the
last — so the machine can only ever have one in flight however many execution units it has. With
four, four chains proceed independently and the core can overlap them. The work is identical; what
differs is how much of it can happen at once.

```{include} _generated/the-cpu-ilp.md
```

The longest program is the fastest, and the shortest is nearly three times slower than it. That is
the answer [ch26](#optimising-code) deferred, and it is as blunt as it looks: on this core, for
this loop, the instruction count predicts the ranking backwards.

Read the second column as well, because it says the returns are running out. Instructions per
cycle rises from one accumulator to four and then falls at eight — the eighth chain is still
worth having in wall-clock terms, and the core is getting less done per cycle while it runs it.
...
Two things in that table are worth reading before the last row. The time per element climbs
steadily as the data becomes less predictable, which is the expected shape. But the mispredict
*rate* does not reach anything like half: data the program picks at random has the predictor wrong
about one time in six, not one in two, because the predictor is not guessing blind — it is doing
much better than chance on data that contains no pattern the chapter put there. And the last two
rows barely differ, so a predictor fed data chosen to be *wrong* every time does no worse than one
fed noise.
...
without the word "derived" next to it — and this one rests on the rate column, so whatever makes
that column flatten at the bottom is inside the number.
...
The timings and the counter readings were measured on the board — time per element for four
accumulator counts with instructions per cycle beside each, and time per element against
misprediction rate for six kinds of data — and [ch24](#measuring)'s discipline applies to every
one of them.
...
**How wide the machine is.** The accumulator experiment never reached the point where another
chain stopped helping — eight was still the fastest of the four tried — so it puts a floor under
how much this core can overlap and no ceiling. The vendor's manual @arm-a76-trm has the issue
width; the measurement has the consequence, and they answer different questions.
...
[ch28](#memory-ordering-on-real-hardware) takes the same machinery and adds more cores. Everything in this chapter assumed
