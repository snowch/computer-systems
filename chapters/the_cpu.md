...
| **Prerequisites** | [ch23](#optimising-code) |
...
[ch23](#optimising-code) ended on a deferral: instruction counts cannot say whether the longer program is the
...
That is [ch23](#optimising-code)'s lesson arriving one chapter later and biting this book: what you wrote and
...
one-accumulator version is the **shortest** program. By [ch23](#optimising-code)'s measure it should be the
best one.
...
number of those, and past that point they spill to memory and [ch22](#the-memory-hierarchy) takes over.
...
The timings and the counter readings are pending. They need the board, and [ch21](#measuring)'s
...
**Whether the compiler will do any of this for you.** [ch23](#optimising-code) established that it does the
...
[ch25](#memory-ordering-on-real-hardware) takes the same machinery and adds a second core. Everything in this chapter assumed
one thread with the whole machine to itself; the next one is about what four of them cost each
other, and it is where [ch17](#locks-and-memory-ordering)'s fences finally get a price.
