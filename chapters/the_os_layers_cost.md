...
short_title: "29 · The OS Layer's Cost on Real Hardware"
---

(the-os-layers-cost)=
# 29 · The OS Layer's Cost on Real Hardware
...
by [ch14](#machine-level-code-on-riscv)'s rule a function that calls nothing needs none — and this function does not
...
next rather than to the call. Finding that in a real program is [ch30](#whole-machine-profiling)'s equipment.
...
**29.2 — What can it not be cheaper than?**
Turn [ch16](#traps-and-system-calls)'s instruction count into a floor, given an IPC and a clock. Divide once at the
...
[ch30](#whole-machine-profiling) stops assuming you know which code to look at. Everything measured so far has been
