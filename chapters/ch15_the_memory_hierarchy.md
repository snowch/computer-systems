...
[ch13](#ch13) left a prediction on the table: two routes over the same array, differing by one
...
```{include} _generated/ch15-levels.md
```
...
```{include} _generated/ch15-line.md
```
...
also, quietly, the answer to a question [ch02](#ch02) raised and could not settle: alignment and
...
[ch07](#ch07) established that every address is a question answered by a walk through three levels
...
```{include} _generated/ch15-vendor.md
```
...
that missed. [ch07](#ch07)'s three levels are three more memory accesses, and this is where that
...
[ch13](#ch13)'s sequential route walks an array in order: every line is used completely before the
...
That is the lesson Part III exists for, and it is worth stating in the form that transfers: **the
...
touched — each a dependent chase, each reported as a minimum over repetitions for [ch14](#ch14)'s
...
write-allocate policies, and the coherence traffic [ch18](#ch18) is about — and measuring reads
...
predictable access pattern may never see any of them. [ch16](#ch16) is where making a pattern
predictable becomes a thing you do deliberately.

## Problems

Three, in `tests/ch15/hierarchy.c`, graded against curves the tests construct with known answers.
...
```bash
python3 -m pytest tests/ch15/test_problem_1_steps.py
```
...
```bash
python3 -m pytest tests/ch15/test_problem_2_line.py
```
...
```bash
python3 -m pytest tests/ch15/test_problem_3_lines.py
```
...
[ch16](#ch16) is the other half of this. Knowing where the data is tells you what a program costs;
