...
[ch17](#ch17) established what a lock is made of and said plainly that what it *costs* is
...
```{include} _generated/ch25-layout.md
```
...
```{include} _generated/ch25-sharing.md
```
...
[ch17](#ch17) printed what a release looks like on both architectures, and the two are not alike.
...
```{include} _generated/ch25-atomics.md
```
...
[ch21](#ch21) asked for a distribution instead of a number. This chapter asks for something else
first: a **prediction**.
...
that relies on not seeing it is broken on a chip that does. [ch17](#ch17)'s problem 10.2 is about
...
```bash
python3 -m pytest tests/ch25/test_problem_1_pairs.py
```
...
```bash
python3 -m pytest tests/ch25/test_problem_2_scaling.py
```
...
```bash
python3 -m pytest tests/ch25/test_problem_3_spelling.py
```
...
[ch26](#ch26) returns to Part IV with the same equipment. Everything xv6 demonstrated structurally
