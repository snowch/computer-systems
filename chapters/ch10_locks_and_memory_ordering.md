...
short_title: "ch10 Locks and Memory Ordering"
---
...
[ch09](#ch09) ended owing an answer. Its census counters are incremented from interrupt handlers
...
```{include} _generated/ch10-race.md
```
...
```{include} _generated/ch10-atomic.md
```
...
[ch16](#ch16) is where being told matters.
...
```{include} _generated/ch10-ordered.md
```
...
```{include} _generated/ch10-publish.md
```
...
```{include} _generated/ch10-primitives.md
```
...
[ch09](#ch09)'s counters are incremented without a lock from several harts and from interrupt
...
matter — the ones in [ch08](#ch08) and [ch09](#ch09) — are the ones fixed by the workload rather
...
not deterministic either: [ch09](#ch09) had already found the console's interrupt count varying
...
Both are questions about hardware, and [ch18](#ch18) asks them on a machine that can answer.
...
[ch09](#ch09)'s console driver used, and they are built on top of the spinlock plus the scheduler
rather than beside it. [ch11](#ch11) has the scheduler and can say what sleeping actually means.

## Problems

Three, in `tests/ch10/locking.c`. The first is the only one in this book that is graded by running
...
```bash
python3 -m pytest tests/ch10/test_problem_1_lock.py
```
...
```bash
python3 -m pytest tests/ch10/test_problem_3_order.py
```
...
[ch11](#ch11) is the other half of what the console driver did. A process that cannot get on
