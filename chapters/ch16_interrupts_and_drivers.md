...
[ch15](#ch15) was about the CPU interrupting itself. A page fault happens *because of* the
...
device is allowed to speak, and the processor takes exactly the trap path [ch13](#ch13) counted —
...
```{figure} _figures/ch16-sources.svg
...
```{include} _generated/ch16-cost.md
```
...
This is [ch13](#ch13)'s rule arriving from a new direction. There the uncountable thing was the
...
machine the chapter runs on. [ch26](#ch26) is on hardware where it is not.
...
innocent process that was running — and this target models none of the last part. [ch26](#ch26)
prices it.

**How often the timer really fires.** The census counts timer interrupts and the book does not
print the number, for [ch13](#ch13)'s reason. It is worth knowing that this count is *steady* here
...
settings are the difference between a measurement and a puzzle, and [ch27](#ch27) has to care.

## Problems

Three. The first two are in `tests/ch16/console.c`; the third is a Python stub you fill in with an
...
```bash
python3 -m pytest tests/ch16/test_problem_1_lost.py
```
...
```bash
python3 -m pytest tests/ch16/test_problem_2_interrupts.py
```
...
[ch17](#ch17) is about the problem this chapter has been carefully stepping around. A handler and
