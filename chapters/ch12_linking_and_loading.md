...
short_title: "ch12 Linking and Loading"
---
...
[ch09](#ch09) left the linker as the stage that "fills in the holes" and moved on. This chapter
...
```{figure} _figures/ch12-segments.svg
...
```{include} _generated/ch12-shape.md
```
...
```{include} _generated/ch12-segment-table.md
```
...
memory — [ch09](#ch09)'s comparison of glibc against xv6's library was about code, and this is the
...
prohibitions are enforced by the page table rather than by convention. [ch14](#ch14) is where that
enforcement becomes a mechanism you can see and [ch15](#ch15) is where breaking it becomes a fault
you can catch.
...
permanently — which is why [ch27](#ch27) spends time on keeping symbols around for the profiler.
...
[ch26](#ch26) touches the cost of the machinery; this chapter does not describe it.
...
copied — and this chapter has no way to measure it. [ch26](#ch26) measures what a process costs to
...
```bash
python3 -m pytest tests/ch12/test_problem_1_reader.py
```
...
```bash
python3 -m pytest tests/ch12/test_problem_2_resolve.py
```
...
```bash
python3 -m pytest tests/ch12/test_problem_3_diagnose.py
```
...
[ch13](#ch13) begins Part IV by asking what happens when that program asks the kernel for
something.
