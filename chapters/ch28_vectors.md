...
```{include} _generated/ch28-loops.md
```
...
```{include} _generated/ch28-scale.md
```
...
```{include} _generated/ch28-sum-f32.md
```
...
```{include} _generated/ch28-running.md
```
...
This is [ch24](#ch24)'s dependence chain again, at a different granularity and with the same
conclusion.
...
```{include} _generated/ch28-speedup.md
```
...
**Whether any of this is where your time is going.** [ch27](#ch27) is the chapter for that, and
...
```bash
python3 -m pytest tests/ch28/test_problem_1_tail.py
```
...
```bash
python3 -m pytest tests/ch28/test_problem_2_reassociation.py
```
...
```bash
python3 -m pytest tests/ch28/test_problem_3_bound.py
```
...
it for the same reason [ch25](#ch25) put two memory models side by side: it solves the tail
