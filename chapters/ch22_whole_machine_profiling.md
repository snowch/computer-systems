...
short_title: "ch22 Whole-Machine Profiling"
---
...
| **Assumes** | that `perf` can sample. ARM PMUs support counter-overflow interrupts as standard, so this works on the reference machine — but most affordable RISC-V cores do not, and a reader following Part IV on one will find this the chapter they cannot run. |
...
```{include} _generated/ch22-census.md
```
...
counted in lines, and there are no unvisited lines. This is [ch20](#ch20)'s observation about coherence reappearing as
...
in the source of that function, and [ch19](#ch19) already established what a branch this lopsided
costs a predictor.
...
```{figure} _figures/ch22-sampling.svg
...
[ch11](#ch11)'s mechanism, in hardware you have already met, doing a job that has nothing to do
...
```{include} _generated/ch22-scatter.md
```
...
```{include} _generated/ch22-skid.md
```
...
```{include} _generated/ch22-profile.md
```
...
enough to collect samples. For anything shorter, the equipment is [ch16](#ch16)'s — repetition and
...
Three, in `tests/ch22/profiler.c`.
...
```bash
python3 -m pytest tests/ch22/test_problem_1_inclusive.py
```
...
```bash
python3 -m pytest tests/ch22/test_problem_2_noise.py
```
...
```bash
python3 -m pytest tests/ch22/test_problem_3_aliasing.py
```
...
[ch23](#ch23) is the last measurement in the book and the narrowest: one loop, one unit, and the
