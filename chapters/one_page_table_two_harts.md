...
has to leave machine mode before any of this means anything, which is why [ch05](#interrupts-and-privilege)
came first.
...
is visible here. [ch23](#the-memory-hierarchy) and [ch26](#memory-ordering-on-real-hardware) are
...
[ch18](#locks-and-memory-ordering) is where the difference between an atomic operation and a
correctly ordered one gets its own chapter.

## Problems

**6.1 — Map a page rather than a gigabyte.**
...
**6.3 — Lose an update the other way round.**
...
[ch07](#a-system-call-of-your-own) turns the boundary this chapter crossed into an interface.
