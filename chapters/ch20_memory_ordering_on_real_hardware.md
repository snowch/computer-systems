...
short_title: "ch20 Memory Ordering on Real Hardware"
---
...
[ch12](#ch12) established what a lock is made of and said plainly that what it *costs* is
...
```{include} _generated/ch20-layout.md
```
...
```{include} _generated/ch20-sharing.md
```
...
[ch12](#ch12) printed what a release looks like on both architectures, and the two are not alike.
...
```{include} _generated/ch20-atomics.md
```
...
[ch16](#ch16) asked for a distribution instead of a number. This chapter asks for something else
first: a **prediction**.
...
that relies on not seeing it is broken on a chip that does. [ch12](#ch12)'s problem 10.2 is about
...
```bash
python3 -m pytest tests/ch20/test_problem_1_pairs.py
```
...
```bash
python3 -m pytest tests/ch20/test_problem_2_scaling.py
```
...
```bash
python3 -m pytest tests/ch20/test_problem_3_spelling.py
```
...
[ch21](#ch21) returns to Part III with the same equipment. Everything xv6 demonstrated structurally
