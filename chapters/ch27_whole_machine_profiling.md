...
```{include} _generated/ch27-census.md
```
...
counted in lines, and there are no unvisited lines. This is [ch25](#ch25)'s observation about coherence reappearing as
...
in the source of that function, and [ch24](#ch24) already established what a branch this lopsided
costs a predictor.
...
```{figure} _figures/ch27-sampling.svg
...
[ch16](#ch16)'s mechanism, in hardware you have already met, doing a job that has nothing to do
...
```{include} _generated/ch27-scatter.md
```
...
```{include} _generated/ch27-skid.md
```
...
```{include} _generated/ch27-profile.md
```
...
enough to collect samples. For anything shorter, the equipment is [ch21](#ch21)'s — repetition and
...
Three, in `tests/ch27/profiler.c`.
...
```bash
python3 -m pytest tests/ch27/test_problem_1_inclusive.py
```
...
```bash
python3 -m pytest tests/ch27/test_problem_2_noise.py
```
...
```bash
python3 -m pytest tests/ch27/test_problem_3_aliasing.py
```
...
[ch28](#ch28) is the last measurement in the book and the narrowest: one loop, one unit, and the
