...
[ch17](#ch17) ended on something that ought to be impossible: a lock held across a context switch.
...
```{include} _generated/ch18-swtch.md
```

Read the first two rows against each other, because the ratio is the point.

[ch13](#ch13)'s trap path saves thirty-one registers. `swtch` saves fourteen. Both are moving "the
state of a thread" and one of them moves less than half as much, which looks like an optimisation
and is not.

**A trap is not a call and a switch is.** The interrupted program of [ch13](#ch13) agreed to
nothing, so everything it might have been using has to be preserved. `swtch` is reached by an
ordinary `jal` from `sched`, which means [ch11](#ch11)'s calling convention has already been
...
This is also the answer to [ch17](#ch17)'s impossibility. The lock is held by a *process*, and the
...
and `pop_off` count nesting per CPU rather than per process, and why [ch17](#ch17) found them
...
```{include} _generated/ch18-census.md
```
...
depends on whether the thing it waited for had already happened. [ch16](#ch16) established this
...
[ch16](#ch16)'s console driver slept, and this is where that gets settled.
...
caches and the TLB full of somebody else's data. [ch26](#ch26) is where that gets a number, and
...
Three, in `tests/ch18/scheduling.c`.

**11.1 — Must the switch save this register?**
Given the register's role under [ch11](#ch11)'s convention and whether the value is still needed,
...
```bash
python3 -m pytest tests/ch18/test_problem_1_save.py
```
...
```bash
python3 -m pytest tests/ch18/test_problem_2_wakeup.py
```
...
```bash
python3 -m pytest tests/ch18/test_problem_3_policy.py
```
...
fourteen lines [ch11](#ch11) sent you to look at without explaining. It has not changed; you have.

[ch19](#ch19) is the last of Part IV and the one with a disk in it. A file system has to survive
