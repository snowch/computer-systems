...
[ch10](#ch10) established what a lock is made of and said plainly that what it *costs* is
...
```{include} _generated/ch18-layout.md
```
...
```{include} _generated/ch18-sharing.md
```
...
[ch10](#ch10) printed what a release looks like on both architectures, and the two are not alike.
...
```{include} _generated/ch18-atomics.md
```
...
[ch14](#ch14) asked for a distribution instead of a number. This chapter asks for something else
first: a **prediction**.
...
that relies on not seeing it is broken on a chip that does. [ch10](#ch10)'s problem 10.2 is about
...
```bash
python3 -m pytest tests/ch18/test_problem_1_pairs.py
```
...
```bash
python3 -m pytest tests/ch18/test_problem_2_scaling.py
```
...
```bash
python3 -m pytest tests/ch18/test_problem_3_spelling.py
```
...
[ch19](#ch19) returns to Part II with the same equipment. Everything xv6 demonstrated structurally
