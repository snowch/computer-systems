...
short_title: "ch14 The File System"
---
...
Every mechanism in Part III so far has assumed the machine keeps running. A lock is held until it
...
```{include} _generated/ch14-amplification.md
```
...
```{include} _generated/ch14-cost.md
```
...
and the disk driver from [ch11](#ch11).
...
That is [ch11](#ch11)'s buffer cache. A block already in memory is not fetched, and a short
...
than to a processor, and shares nothing with [ch12](#ch12)'s fences but a name.
...
Three, in `tests/ch14/filesystem.c`.
...
```bash
python3 -m pytest tests/ch14/test_problem_1_writes.py
```
...
```bash
python3 -m pytest tests/ch14/test_problem_2_crash.py
```
...
```bash
python3 -m pytest tests/ch14/test_problem_3_ordering.py
```
...
[ch15](#ch15) is the hinge. Everything Part III has established is about what a program *does*, on a
...
whole of Part IV exists to answer.
