...
| **Prerequisites** | [ch11](#machine-level-code-on-riscv) |
...
[ch09](#what-a-computer-does-with-a-program) left the linker as the stage that "fills in the holes" and moved on. This chapter
...
memory — [ch09](#what-a-computer-does-with-a-program)'s comparison of glibc against xv6's library was about code, and this is the
...
prohibitions are enforced by the page table rather than by convention. [ch14](#virtual-memory) is where that
enforcement becomes a mechanism you can see and [ch15](#page-faults-as-a-feature) is where breaking it becomes a fault
you can catch.
...
permanently — which is why [ch27](#whole-machine-profiling) spends time on keeping symbols around for the profiler.
...
[ch26](#the-os-layers-cost) touches the cost of the machinery; this chapter does not describe it.
...
copied — and this chapter has no way to measure it. [ch26](#the-os-layers-cost) measures what a process costs to
...
[ch13](#traps-and-system-calls) begins Part IV by asking what happens when that program asks the kernel for
something.
