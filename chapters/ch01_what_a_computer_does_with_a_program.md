...
short_title: "ch01 What a Computer Does With a Program"
---
...
Separating the two is the skill the rest of Part I is built on, because you cannot ask what a
...
```{figure} _figures/ch01-stages.svg
...
```{include} _generated/ch01-folded.md
```
...
```{include} _generated/ch01-counted.md
```
...
[ch02](#ch02)'s subject, arriving early and uninvited, as it tends to.
...
```{include} _generated/ch01-stage-sizes.md
```
...
```{include} _generated/ch01-linking.md
```
...
identical — it is the cost of everything `printf` is prepared to do if asked. [ch05](#ch05) opens
...
worth listening to, and the board is not attached to this chapter. [ch13](#ch13) puts that
question properly, and [ch16](#ch16) answers it.
...
instructions is not automatically cheaper than one that compiles to ten. [ch15](#ch15) shows a
...
`tests/ch01/problem_1_stages.py` gives you fragments of the four files and asks which program
...
```bash
python3 -m pytest tests/ch01/test_problem_1_stages.py
```
...
```bash
python3 -m pytest tests/ch01/test_problem_2_ripple.py
```
...
```bash
python3 -m pytest tests/ch01/test_problem_3_unfold.py
```
...
reads: sections, symbols and relocations, all of which [ch05](#ch05) takes apart properly. It is
...
[ch02](#ch02) goes back to the thing the compiler was manipulating in the first place: what a
