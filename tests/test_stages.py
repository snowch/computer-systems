"""Claims Part II makes about emitted code, asserted so a compiler changing its mind fails CI.

ch04's program says the same thing on both targets and the compiler folds one route away; ch03's
...
def test_both_targets_print_the_same_facts():
    """The claim ch04 closes on. Only the first line, naming the world, may differ."""
...
def test_only_one_of_the_two_routes_survives_to_run_time():
    """ch04's central contrast, asserted so that a compiler changing its mind fails the build.
    ...
    assert folded.instructions < counted.instructions, (
        "ch04 says the folded route is the shorter one:\n" + folded.text
    )
    assert "2016" in folded.text or "0x7e0" in folded.text, (
        "ch04 says the compiler wrote the answer into the instruction stream:\n" + folded.text
    )
...
def test_the_walk_measures_the_toolchain_and_not_the_checkout(tmp_path):
    """ch04's stage sizes must be the same in two clones of the same commit. They were not.
    ...
    assert walks[0] == walks[1], (
        "the same commit measured differently from two directories, so ch04's stage sizes are "
        f"partly a statement about where the tree lives:\n{walks[0]}\n{walks[1]}"
    )


def test_the_trap_probe_matches_the_program_it_counts():
    """ch08's census is only reproducible while the runner and the workload agree.
