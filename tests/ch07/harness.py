...
def build(build_dir: Path) -> Path:
    return compile_program([WALK], build_dir / "ch07walk", NATIVE).path
...
def tables_needed(build_dir: Path, runs: list[tuple[int, int]]) -> int:
    ...
    model = compile_program(
        [MODEL, MODEL_LIB], build_dir / "ch07model", NATIVE, includes=["sysfs/include"]
    ).path
