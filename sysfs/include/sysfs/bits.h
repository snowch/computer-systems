 ...
 *
 * No <stdint.h>. These are compiled for the host target, but the virtual-memory chapter wants the alignment helpers
 ...
 * book's architectures, which the prerequisites chapter measured rather than assumed.
 */
 ...
 * the alternative is returning a lie and the representing-information chapter is about not doing that. */
unsigned long sysfs_round_up_pow2(unsigned long value);

/* Whether `value` is a multiple of `alignment`. `alignment` must be a power of two — the virtual-memory chapter relies
 * on this and its alignments always are. */
